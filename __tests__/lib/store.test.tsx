...
describe('AppProvider store behaviors (SEC-04 / SEC-05)', () => {
  ...
  // -------------------------------------------------------------------------
  test('SEC-04: loadAll re-fires when eventId changes from 1 to 2', async () => {
  ...
  // -------------------------------------------------------------------------
  test('SEC-05: realtime subscription includes event_id=eq.1 filter on postgres_changes', async () => {
  ...
  // -------------------------------------------------------------------------
  test('SEC-05: realtime channel is torn down and rebuilt when eventId changes', async () => {
    const { rerender } = renderWithProvider(1)
  ...
  // -------------------------------------------------------------------------
  test('SEC-04: null guard -- loadAll does not call getEvent when eventId is undefined', async () => {
    const db = await getDbMock()
    const getEvent = db.getEvent as ReturnType<typeof vi.fn>
    ...
    // getEvent should NOT be called when no eventId is provided
