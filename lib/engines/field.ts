...
export async function runFieldConflictEngine(
  ...
  if (!games || games.length === 0) {
    return {
      ...
      stats: { gamesScanned: 0, fieldsScanned: 0, blocksChecked: 0, overlapCount: 0, blockedCount: 0, cascadeCount: 0 },
  ...
  for (const [fieldId, fieldGames] of byField.entries()) {
    const sorted = [...fieldGames].sort((a, b) =>
  ...
  for (const game of games) {
    ...
    for (const block of (fieldBlocks ?? [])) {
      ...
      const blockStart = timeToMinutes(
        new Date(block.starts_at).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true })
      )
      const blockEnd = timeToMinutes(
        new Date(block.ends_at).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true })
      )
        ...
        conflicts.push({
          ...
          resolutionOptions: [
            ...
            },
            {
              ...
              params: { game_id: game.id, after_time: new Date(block.ends_at).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true }) },
            },
          ],
  ...
  try {
    switch (action) {
      ...
      case 'delay_game': {
        ...
        // Get current time
        const { data: game } = await sb.from('games').select('scheduled_time').eq('id', Number(game_id)).single()
        ...
        await sb.from('games').update({ scheduled_time: newTime, status: 'Delayed' }).eq('id', Number(game_id))
        await sb.from('ops_log').insert({
          event_id:    EVENT_ID,
        ...
        return { success: true, message: `Game #${game_id} delayed ${delay_minutes} min to ${newTime}` }
      }

      case 'move_to_field': {
        // For now mark as needing manual assignment — field selection done in UI
        return { success: true, message: 'Open the Schedule tab to reassign this game to another field' }
      }

      case 'swap_games': {
        const { game_id_a, game_id_b } = params
        const { data: gameA } = await sb.from('games').select('scheduled_time, field_id').eq('id', Number(game_id_a)).single()
        const { data: gameB } = await sb.from('games').select('scheduled_time, field_id').eq('id', Number(game_id_b)).single()
        if (!gameA || !gameB) return { success: false, message: 'Games not found' }
        await sb.from('games').update({ scheduled_time: gameB.scheduled_time }).eq('id', Number(game_id_a))
        await sb.from('games').update({ scheduled_time: gameA.scheduled_time }).eq('id', Number(game_id_b))
        await sb.from('ops_log').insert({
          event_id:    EVENT_ID,
...
export async function runFullConflictScan(eventDateId: number) {
  ...
  const [fieldResult] = await Promise.all([
    runFieldConflictEngine(eventDateId),
  ])
