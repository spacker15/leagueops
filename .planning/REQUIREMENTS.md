...
- [ ] **SEC-03**: All engine modules (`referee.ts`, `weather.ts`, `field.ts`, `eligibility.ts`, `unified.ts`) accept a server-side Supabase client parameter instead of importing browser client
...
- [ ] **EVT-01**: Admin can search for a venue/complex via Google Maps autocomplete when creating an event
- [ ] **EVT-02**: Complex location (lat/lng, address, place ID) saved to event record
...
- [ ] **EVT-05**: System generates a QR code for the registration link that admin can download/share
- [ ] **EVT-06**: Registration link/QR includes event slug for direct routing
...
- [ ] **NOT-01**: Notification queue table receives entries from engines and workflows — processed asynchronously via Supabase Edge Function
...
- [ ] **NOT-05**: Email delivery via Resend (3,000/month free tier)
...
- [ ] **NOT-08**: Deduplication prevents notification storms (e.g., single lightning alert doesn't trigger hundreds of messages)
