...
| Autocomplete route uses server-only key | `grep -q "GOOGLE_MAPS_API_KEY" route.ts` + no NEXT_PUBLIC_ prefix | Both routes use `process.env.GOOGLE_MAPS_API_KEY` (non-public) | ✓ PASS |
