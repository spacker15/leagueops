...
interface Props {
  token: string
  invitedRole: 'coach' | 'assistant_coach'
  invitedEmail: string
  programName: string
  eventName: string
  primaryColor: string
...
export function CoachJoinClient({
  ...
  const label = invitedRole === 'assistant_coach' ? 'Assistant Coach' : 'Coach'
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const res = await fetch('/api/join/coach', {
      ...
      body: JSON.stringify({ token, first_name: firstName, last_name: lastName, email, password }),
    })
      ...
      >
        <div className="bg-[#081428] border border-green-800/50 rounded-2xl p-8 max-w-sm w-full text-center">
          ...
          <div className="font-cond text-[13px] text-[#5a6e9a] mb-1">
            You&apos;ve been added as {label.toLowerCase()} for
          </div>
          <div className="font-cond font-black text-[15px] text-white mb-1">{programName}</div>
          <div className="font-cond text-[12px] text-[#5a6e9a] mb-4">{eventName}</div>
          <a
            href="/"
            className="block w-full font-cond font-black text-[13px] tracking-[.1em] py-3 rounded-xl text-white text-center transition-all"
        ...
        <div className="bg-[#081428] border border-[#1a2d50] border-t-0 rounded-b-2xl p-8">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              ...
              <div className="font-cond text-[10px] text-[#5a6e9a] mt-0.5">{eventName}</div>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              ...
              style={{ background: primaryColor }}
            >
              {submitting ? 'CREATING ACCOUNT...' : `JOIN AS ${label.toUpperCase()}`}
            </button>
          </form>
