...
import { DollarSign, Plus, CheckCircle, Clock, AlertCircle, TrendingUp, X, ChevronDown } from 'lucide-react'
import type { RegistrationFee, TeamPayment, PaymentEntry, PaymentMethod, PaymentStatus } from '@/types'
...
function RecordPaymentModal({ payment, onClose, onSaved }: RecordPaymentModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
      <div className="w-[420px] rounded-xl shadow-2xl border border-[#1a2d50]" style={{ background: '#061428' }}>
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#1a2d50]">
          <div>
            <div className="font-cond text-[13px] font-black tracking-wider text-white">RECORD PAYMENT</div>
            <div className="font-cond text-[11px] text-muted">{payment.team_name} — {payment.division}</div>
          </div>
        ...
        <div className="p-5 space-y-4">
          ...
          <div>
            ...
            <input className={inp} type="number" min="0.01" step="0.01"
              ...
              placeholder={payment.balance.toFixed(2)} autoFocus />
          </div>
          ...
          <div>
            ...
            <input className={inp} type="date" value={paidAt} onChange={e => setPaidAt(e.target.value)} />
          </div>

          {/* Reference */}
          <div>
            <label className="font-cond text-[10px] text-muted block mb-1">Reference # (check #, transaction ID, etc.)</label>
            <input className={inp} value={ref} onChange={e => setRef(e.target.value)} placeholder="Optional" />
          </div>
...
function AddTeamPaymentModal({ eventId, fees, teams, onClose, onSaved }: AddTeamPaymentModalProps) {
  ...
  async function submit() {
    ...
    if (isNaN(amt) || amt < 0) { toast.error('Enter a valid amount due'); return }
    setSaving(true)
    ...
    setSaving(false)
    if (!res.ok) { toast.error('Failed to add team'); return }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
      <div className="w-[400px] rounded-xl shadow-2xl border border-[#1a2d50]" style={{ background: '#061428' }}>
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#1a2d50]">
        ...
        <div className="p-5 space-y-4">
          <div>
            <label className="font-cond text-[10px] text-muted block mb-1">Select Registered Team (optional)</label>
            <select className={inp} value={teamId} onChange={e => handleTeamSelect(e.target.value)}>
          ...
          <div>
            ...
            <input className={inp} value={teamName} onChange={e => setTeamName(e.target.value)} placeholder="e.g. Knights Blue" />
          </div>
          <div>
            <label className="font-cond text-[10px] text-muted block mb-1">Division</label>
            <input className={inp} value={division} onChange={e => setDivision(e.target.value)} placeholder="e.g. U12" />
          </div>
          <div>
            <label className="font-cond text-[10px] text-muted block mb-1">Amount Due *</label>
            <input className={inp} type="number" min="0" step="0.01" value={amtDue}
              onChange={e => setAmtDue(e.target.value)} placeholder="0.00" />
          </div>
        </div>
        <div className="flex justify-end gap-2 px-5 pb-5">
          <button onClick={onClose}
...
export function PaymentsTab() {
  ...
  const [history, setHistory]   = useState<(PaymentEntry & { team_name?: string; team_payment?: TeamPayment })[]>([])
  ...
  async function loadTeamHistory(tpId: number) {
    if (teamHistory[tpId]) { setExpandedHistory(expandedHistory === tpId ? null : tpId); return }
  ...
  async function saveFee(division: string) {
    ...
    const res = await fetch('/api/registration-fees', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ event_id: eventId, division, amount: val, ...(existing ? { id: existing.id } : {}) }),
    })
    setSavingFee(null)
    if (!res.ok) { toast.error('Failed to save fee'); return }
    toast.success(`Fee saved for ${division}`)
    setFeeEdits(e => { const n = { ...e }; delete n[division]; return n })
    load()
  }
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-5">
        <div className="flex items-center gap-2">
          ...
          <span className="font-cond text-[11px] font-bold tracking-widest text-muted uppercase">Payments</span>
      ...
      <div className="flex gap-1 mb-5 border-b border-[#1a2d50]">
        {SUBTABS.map(t => (
          <button key={t.id} onClick={() => setSubTab(t.id)}
            className={cn('font-cond text-[11px] font-black tracking-[.1em] px-4 py-2.5 transition-colors border-b-2 -mb-[2px]',
              subTab === t.id
      ...
      {loading && (
        <div className="text-center py-16 text-muted font-cond text-[13px]">Loading payment data…</div>
      )}
        ...
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              ...
              { label: 'Collected',    value: fmt(totalPaid),   icon: TrendingUp,   color: 'text-green-400' },
              { label: 'Outstanding',  value: fmt(outstanding), icon: AlertCircle,  color: outstanding > 0 ? 'text-yellow-400' : 'text-green-400' },
              { label: 'Paid in Full', value: `${paidCount} / ${payments.length}`, icon: CheckCircle, color: 'text-green-400' },
            ].map(({ label, value, icon: Icon, color }) => (
              <div key={label} className="bg-[#081428] border border-[#1a2d50] rounded-xl p-4">
                <div className="flex items-center gap-2 mb-2">
                  <Icon size={14} className="text-muted" />
                  <span className="font-cond text-[10px] font-black tracking-[.12em] text-muted uppercase">{label}</span>
                </div>
          ...
          {divisions.length > 0 && (
            <div className="bg-[#081428] border border-[#1a2d50] rounded-xl overflow-hidden">
              <div className="px-4 py-3 border-b border-[#1a2d50]">
                <span className="font-cond text-[10px] font-black tracking-[.12em] text-muted uppercase">By Division</span>
              </div>
              <table className="w-full">
                <thead>
                  <tr className="border-b border-[#1a2d50]">
                    {['Division', 'Teams', 'Total Due', 'Collected', 'Outstanding', 'Paid'].map(h => (
                      <th key={h} className="font-cond text-[10px] font-black tracking-[.1em] text-muted uppercase text-left px-4 py-2.5">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                    ...
                    return (
                      <tr key={div} className="border-b border-[#0d1a2e] last:border-0">
                        <td className="px-4 py-3">
                          <span className="font-cond font-bold text-[12px] text-blue-300 bg-[#1a2d50] px-2 py-0.5 rounded">{div}</span>
                        </td>
                        <td className="px-4 py-3 font-cond text-[12px] text-white">{divPay.length}</td>
                        <td className="px-4 py-3 font-cond text-[12px] text-white">{fmt(due)}</td>
                        <td className="px-4 py-3 font-cond text-[12px] text-green-400">{fmt(paid)}</td>
                        <td className="px-4 py-3 font-cond text-[12px] text-yellow-400">{fmt(due - paid)}</td>
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2">
          ...
          {payments.length === 0 && (
            <div className="text-center py-10 text-muted font-cond text-[13px]">
              No teams yet — click <strong className="text-white/50">ADD TEAM</strong> or configure fees first
            </div>
          )}
        </div>
      )}
      ...
      {!loading && subTab === 'teams' && (
        <div className="space-y-4">
          ...
          <div className="flex gap-3 items-center">
            <select className={cn(inp, 'w-40')} value={filterDiv} onChange={e => setFilterDiv(e.target.value)}>
              <option value="">All Divisions</option>
              {divisions.map(d => <option key={d} value={d}>{d}</option>)}
            </select>
            <select className={cn(inp, 'w-40')} value={filterStatus} onChange={e => setFilterStatus(e.target.value as PaymentStatus | '')}>
              <option value="">All Statuses</option>
              {(['pending','partial','paid','waived','refunded'] as PaymentStatus[]).map(s => (
                <option key={s} value={s}>{s.charAt(0).toUpperCase() + s.slice(1)}</option>
              ))}
            </select>
            <span className="font-cond text-[11px] text-muted">{filteredPayments.length} teams</span>
          </div>

          {filteredPayments.length === 0 ? (
            <div className="text-center py-10 text-muted font-cond text-[13px]">No teams match filters</div>
          ) : (
            <div className="bg-[#081428] border border-[#1a2d50] rounded-xl overflow-hidden">
              <table className="w-full">
                <thead>
                ...
                <tbody>
                  {filteredPayments.map(p => (
                    <>
                      <tr key={p.id} className="border-b border-[#0d1a2e] last:border-0 group hover:bg-white/[0.02]">
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-1.5">
                            <button onClick={() => loadTeamHistory(p.id)}
                              className="text-muted hover:text-white transition-colors">
                              <ChevronDown size={12} className={cn('transition-transform', expandedHistory === p.id ? 'rotate-180' : '')} />
                            </button>
                            <span className="font-cond font-bold text-[13px] text-white">{p.team_name}</span>
                          </div>
                        </td>
                        <td className="px-4 py-3">
                          <span className="font-cond text-[11px] font-bold px-2 py-0.5 rounded bg-[#1a2d50] text-blue-300">{p.division || '—'}</span>
                        </td>
                        <td className="px-4 py-3 font-cond text-[12px] text-white">{fmt(Number(p.amount_due))}</td>
                        <td className="px-4 py-3 font-cond text-[12px] text-green-400">{fmt(Number(p.amount_paid))}</td>
                        <td className="px-4 py-3 font-cond text-[12px] text-yellow-400">{fmt(Number(p.balance ?? (Number(p.amount_due) - Number(p.amount_paid))))}</td>
                        <td className="px-4 py-3">
                          <span className={cn('font-cond text-[10px] font-black tracking-wide px-2 py-0.5 rounded uppercase', STATUS_COLORS[p.status])}>
                            {p.status}
                          </span>
                        </td>
                        <td className="px-4 py-3">
                          <button onClick={() => setRecordTarget(p)}
                        ...
                        <tr key={`hist-${p.id}`} className="border-b border-[#0d1a2e]">
                          <td colSpan={7} className="px-6 py-3 bg-[#040d1c]">
                            {(teamHistory[p.id] ?? []).length === 0 ? (
                              <span className="font-cond text-[11px] text-muted">No payments recorded yet</span>
                            ) : (
                              <div className="space-y-1">
                                {(teamHistory[p.id] ?? []).map(e => (
                                  <div key={e.id} className="flex items-center gap-4 font-cond text-[11px]">
                                    <span className="text-green-400 font-bold">{fmt(Number(e.amount))}</span>
                                    <span className="text-muted">{METHOD_LABELS[e.payment_method]}</span>
                                    {e.reference_number && <span className="text-white/40">#{e.reference_number}</span>}
                                    <span className="text-muted">{dateFmt(e.paid_at)}</span>
                                    {e.notes && <span className="text-white/50 italic">{e.notes}</span>}
                                  </div>
                                ))}
                              </div>
                            )}
      ...
      {!loading && subTab === 'fees' && (
        <div className="space-y-4">
          <div className="font-cond text-[11px] text-muted">
            Set a registration fee per division. When you add a team, the fee will be pre-filled automatically.
          </div>
          ...
          <div className="bg-[#081428] border border-[#1a2d50] rounded-xl overflow-hidden">
            <table className="w-full">
              <thead>
                <tr className="border-b border-[#1a2d50]">
                  <th className="font-cond text-[10px] font-black tracking-[.1em] text-muted uppercase text-left px-4 py-2.5">Division</th>
                  <th className="font-cond text-[10px] font-black tracking-[.1em] text-muted uppercase text-left px-4 py-2.5 w-48">Fee Amount (USD)</th>
              ...
              <tbody>
                    ...
                    <tr key={div} className="border-b border-[#0d1a2e] last:border-0">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1">
                          ...
                          <input className={cn(inp, 'w-32')} type="number" min="0" step="0.01"
                            value={editVal}
                      ...
                      <td className="px-4 py-3">
                        {feeEdits[div] !== undefined && (
                          <button onClick={() => saveFee(div)} disabled={savingFee === div}
                ...
                {divisions.length === 0 && (
                  <tr>
                    <td colSpan={3} className="px-4 py-8 text-center font-cond text-[12px] text-muted">
      ...
      {!loading && subTab === 'history' && (
        <div>
          {history.length === 0 ? (
            <div className="text-center py-10 text-muted font-cond text-[13px]">No payment entries yet</div>
          ) : (
            <div className="bg-[#081428] border border-[#1a2d50] rounded-xl overflow-hidden">
              <table className="w-full">
                <thead>
                ...
                <tbody>
                  {history.map(e => (
                    <tr key={e.id} className="border-b border-[#0d1a2e] last:border-0">
                      <td className="px-4 py-3 font-cond text-[12px] text-muted">{dateFmt(e.paid_at)}</td>
                      <td className="px-4 py-3 font-cond text-[12px] text-white">
                        {payments.find(p => p.id === e.team_payment_id)?.team_name ?? '—'}
                      </td>
                      <td className="px-4 py-3 font-cond text-[12px] font-bold text-green-400">{fmt(Number(e.amount))}</td>
                      <td className="px-4 py-3 font-cond text-[12px] text-white">{METHOD_LABELS[e.payment_method]}</td>
                      <td className="px-4 py-3 font-cond text-[12px] text-muted">{e.reference_number || '—'}</td>
                      <td className="px-4 py-3 font-cond text-[12px] text-white/50">{e.notes || '—'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
