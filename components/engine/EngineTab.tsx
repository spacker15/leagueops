...
export function EngineTab() {
  ...
  const [selectedFields, setSelectedFields] = useState<number[]>(state.fields.slice(0,6).map(f => f.id))
  ...
  function runEngine() {
    if (teams.length < 2) { toast.error('Add at least 2 teams'); return }
    if (selectedFields.length === 0) { toast.error('Select at least one field'); return }
    const fieldNames = selectedFields.map(id => state.fields.find(f => f.id === id)?.name ?? `Field ${id}`)
  ...
  async function importSchedule() {
    if (!generated || !currentDate) { toast.error('No schedule or no event date selected'); return }
    setImporting(true)
    let added = 0
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        <div>
          <div className="bg-surface-card border border-border rounded-md p-4 mb-4">
            <div className="font-cond font-black text-[13px] tracking-wide mb-3">TEAMS & DIVISIONS</div>
            <div className="grid grid-cols-2 gap-3 mb-3">
              <FormField label="Division">
                <select
                  ...
                  value={engDivision} onChange={e => setEngDivision(e.target.value)}
                >
                  {divisions.length > 0
                    ? divisions.map(d => <option key={d} value={d}>{d}</option>)
                    : ['U10','U12','U14','U16','U18'].map(d => <option key={d} value={d}>{d}</option>)
                  }
                </select>
              </FormField>
              <FormField label="Add Team">
                <div className="flex gap-1">
                  ...
                  />
                  <Btn size="sm" variant="primary" onClick={addTeam}>+</Btn>
                </div>
              </FormField>
            </div>
              ...
              {teams.length === 0
                ...
                : teams.map(t => (
                  <span key={t.name}
                    ...
                    <button onClick={() => removeTeam(t.name)} className="text-muted hover:text-red-400 transition-colors">
                      <X size={10} />
                    </button>
                  </span>
                ))
          ...
          <div className="bg-surface-card border border-border rounded-md p-4 mb-4">
            <div className="font-cond font-black text-[13px] tracking-wide mb-3">SCHEDULE PARAMETERS</div>
            <div className="grid grid-cols-2 gap-3">
              <FormField label="Games per Team">
                <select className="bg-surface border border-border text-white px-2.5 py-1.5 rounded text-[13px] outline-none focus:border-blue-400"
                  value={gamesPerTeam} onChange={e => setGamesPerTeam(Number(e.target.value))}>
                  {[1,2,3,4,5].map(n => <option key={n} value={n}>{n} game{n>1?'s':''}</option>)}
                </select>
              </FormField>
              <FormField label="Game Duration">
                <select className="bg-surface border border-border text-white px-2.5 py-1.5 rounded text-[13px] outline-none focus:border-blue-400"
                  value={duration} onChange={e => setDuration(Number(e.target.value))}>
                  {[40,50,60,70,80].map(n => <option key={n} value={n}>{n} min</option>)}
                </select>
              </FormField>
              ...
              <FormField label="Min Rest (min)">
                <input type="number" min={30} value={minRest} onChange={e => setMinRest(Number(e.target.value))}
                  className="bg-surface border border-border text-white px-2.5 py-1.5 rounded text-[13px] outline-none focus:border-blue-400" />
              </FormField>
            </div>
          </div>
          ...
          <div className="bg-surface-card border border-border rounded-md p-4 mb-4">
            <div className="font-cond font-black text-[13px] tracking-wide mb-2">AVAILABLE FIELDS</div>
                ...
                <div className="flex-1 overflow-y-auto font-mono text-[11px] bg-black/30 rounded p-3 mb-3 space-y-0.5">
                  {generated.map((g, i) => (
                    <div key={i} className="flex gap-2">
                      ...
                      <span className="text-muted text-[9px] w-12 text-right shrink-0">{g.division}</span>
                    </div>
                  ))}
                </div>
                <Btn
