...
import {
  Zap, AlertTriangle, CheckCircle, XCircle, Clock, RefreshCw,
  ChevronRight, Flame, Wind, Activity, Users, Shield,
  FileText, ArrowUpRight, Radio, Play, Pause,
} from 'lucide-react'

type CmdTab = 'alerts' | 'feed' | 'fields' | 'handoff'

interface OpsLogEntry {
  id: number; message: string; log_type: string
  occurred_at: string; source: string | null; entity_type: string | null
}
...
export function CommandCenter() {
  ...
  const loadAlerts = useCallback(async () => {
    ...
    const { data } = showResolved ? await query.order('created_at', { ascending: false }).limit(100)
    ...
    const sorted = [...(data ?? [])].sort((a, b) =>
      ...
      new Date((b as OpsAlert).created_at).getTime() - new Date((a as OpsAlert).created_at).getTime()
    )
  ...
  const loadFeed = useCallback(async () => {
    const sb = createClient()
    const { data } = await sb.from('ops_log').select('*').eq('event_id', 1)
      .order('occurred_at', { ascending: false }).limit(80)
    setFeed((data as OpsLogEntry[]) ?? [])
  }, [])

  useEffect(() => { loadAlerts(); loadFeed() }, [loadAlerts, loadFeed])

  // Realtime subscriptions
  useEffect(() => {
    const sb = createClient()
    const alertSub = sb.channel('cmd-alerts')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'ops_alerts' }, () => loadAlerts())
      .subscribe()
    const feedSub = sb.channel('cmd-feed')
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'ops_log' }, (payload) => {
    ...
    const gameSub = sb.channel('cmd-games')
  ...
  async function handleRunAll() {
    ...
    try {
      ...
      else toast(`${result.alerts_created} new alerts, ${result.alerts_escalated} escalated`, { icon: '⚠️', duration: 4000 })
    } catch (err: any) {
    ...
    <div>
      ...
      <div className="flex items-center gap-0 mb-4 border-b border-border">
        {[
          ...
          <button key={t.id} onClick={() => setTab(t.id as CmdTab)}
            className={cn(
        ...
        <div>
            ...
            <div className="flex gap-2">
              <button onClick={() => setShowResolved(s => !s)}
                className={cn('font-cond text-[11px] font-bold px-3 py-1.5 rounded border transition-colors',
                  showResolved ? 'bg-navy border-blue-400 text-white' : 'bg-surface-card border-border text-muted hover:text-white'
                )}>
          ...
          {alerts.length === 0 ? (
            <div className="flex flex-col items-center py-16 gap-3">
              ...
              <div className="font-cond font-black text-[20px] text-green-400">ALL SYSTEMS CLEAR</div>
              <div className="font-cond text-[12px] text-muted">Run All Engines to scan for new conflicts</div>
            </div>
          ) : (
            <div className="space-y-3">
              {alerts.map(a => <AlertCard key={a.id} alert={a} resolving={resolvingId === a.id}
        ...
        <div>
          <div className="flex justify-between items-center mb-3">
            <div className="flex items-center gap-2">
              ...
              <span className="font-cond text-[11px] text-white font-bold">LIVE OPERATIONS FEED</span>
            </div>
            <div className="flex gap-2">
              <button onClick={() => setFeedPaused(p => !p)}
                className={cn('flex items-center gap-1.5 font-cond text-[11px] font-bold px-3 py-1.5 rounded border transition-colors',
                  feedPaused ? 'bg-yellow-800/40 border-yellow-700/50 text-yellow-300' : 'bg-surface-card border-border text-muted hover:text-white'
                )}>
                {feedPaused ? <><Play size={11} /> RESUME</> : <><Pause size={11} /> PAUSE</>}
              </button>
              <button onClick={loadFeed} className="font-cond text-[11px] text-muted hover:text-white px-2 py-1.5">
                <RefreshCw size={12} />
              </button>
            </div>
          </div>

          <div ref={feedRef} className="bg-surface-card border border-border rounded-xl overflow-hidden">
            {feed.length === 0 ? (
        ...
        <div>
          ...
          <div className="grid grid-cols-[repeat(auto-fill,minmax(240px,1fr))] gap-4">
            {state.fields.map(field => {
              const fieldGames = (gamesByField[field.id] ?? [])
      ...
      {tab === 'handoff' && (
        <div>
          <div className="flex justify-between items-center mb-4">
            <div>
              <div className="font-cond font-black text-[14px] text-white mb-0.5">SHIFT HANDOFF LOG</div>
              <div className="font-cond text-[11px] text-muted">
                Auto-generates a summary of the last hour for incoming operations staff.
              </div>
            </div>
            <button onClick={handleGenerateHandoff} disabled={generatingHandoff}
          ...
          {handoff ? (
            <div className="bg-surface-card border border-border rounded-xl p-5">
              <div className="font-cond text-[11px] text-muted mb-3 flex justify-between">
                ...
                }} className="text-blue-300 hover:text-white">Copy</button>
              </div>
              <pre className="font-mono text-[12px] text-white whitespace-pre-wrap leading-relaxed">{handoff}</pre>
            </div>
          ) : (
            <HandoffHistory />
          )}
...
// ─── Alert Card ───────────────────────────────────────────────
function AlertCard({ alert: a, resolving, onResolve }: {
  alert: OpsAlert; resolving: boolean
  ...
  const severityConfig = {
    critical: { bg: 'bg-red-900/20 border-red-700/60', badge: 'bg-red-700 text-white', icon: <AlertTriangle size={16} className="text-red-400 flex-shrink-0" />, label: 'CRITICAL' },
    warning:  { bg: 'bg-yellow-900/15 border-yellow-700/50', badge: 'bg-yellow-700/80 text-white', icon: <AlertTriangle size={16} className="text-yellow-400 flex-shrink-0" />, label: 'WARNING' },
    info:     { bg: 'bg-blue-900/10 border-blue-700/30', badge: 'bg-blue-800 text-white', icon: <Activity size={16} className="text-blue-400 flex-shrink-0" />, label: 'INFO' },
    resolved: { bg: 'bg-green-900/10 border-green-700/30 opacity-60', badge: 'bg-green-800 text-white', icon: <CheckCircle size={16} className="text-green-400 flex-shrink-0" />, label: 'RESOLVED' },
  }
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-2">
              <div className="font-cond font-black text-[15px] text-white leading-tight">{a.title}</div>
              <div className="flex items-center gap-2 flex-shrink-0">
                ...
                <span className={cn('font-cond text-[9px] font-black px-2 py-0.5 rounded tracking-widest', cfg.badge)}>
                  {cfg.label}
                </span>
            ...
            {a.description && <div className="font-cond text-[12px] text-muted mt-0.5 leading-snug">{a.description}</div>}

            {/* Resolution suggestion */}
            {a.resolution_suggestion && !a.resolved && (
              <div className="mt-2.5 bg-black/20 rounded-lg px-3 py-2 border border-border/40">
                <div className="font-cond text-[9px] font-bold tracking-widest text-muted uppercase mb-0.5">RECOMMENDED ACTION</div>
                <div className="font-cond text-[12px] text-blue-300 leading-snug">{a.resolution_suggestion}</div>
                {a.resolution_action && (
        ...
        {!a.resolved && (
          <div className="mt-3 flex gap-2 items-center">
            {!expanded ? (
              <>
                {a.resolution_action ? (
                  <button onClick={() => onResolve(a.resolution_suggestion ?? undefined)} disabled={resolving}
              ...
              <div className="flex gap-2 items-center flex-1">
                <input className="flex-1 bg-surface border border-border text-white px-2.5 py-2 rounded text-[12px] outline-none focus:border-blue-400"
                  placeholder="Resolution note (optional)" value={note} onChange={e => setNote(e.target.value)} />
                <button onClick={() => { onResolve(note || undefined); setExpanded(false) }} disabled={resolving}
                ...
                <button onClick={() => setExpanded(false)} className="text-muted hover:text-white text-[11px]">✕</button>
              </div>
            )}
          </div>
        )}
...
function FeedEntry({ entry, isNew }: { entry: OpsLogEntry; isNew: boolean }) {
  const time = new Date(entry.occurred_at).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', second: '2-digit' })
  ...
  const sourceLabel = entry.source ? sourceLabels[entry.source] ?? entry.source.toUpperCase() : null

  return (
    <div className={cn('flex items-start gap-3 px-4 py-2.5 transition-colors hover:bg-white/5',
...
// ─── Field Card ───────────────────────────────────────────────
function FieldCard({ field, games, liveGame, fieldAlerts }: {
  field: any; games: any[]; liveGame: any; fieldAlerts: OpsAlert[]
}) {
  const [expanded, setExpanded] = useState(false)
  ...
  return (
    <div className={cn('rounded-xl border overflow-hidden',
      hasCritical ? 'border-red-700/60 bg-red-900/10' :
      hasWarning  ? 'border-yellow-700/50 bg-yellow-900/10' :
      liveGame    ? 'border-green-700/50 bg-green-900/10' :
        ...
        <div className="px-4 py-3 border-b border-border/50">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              {liveGame && !hasCritical && <div className="w-2 h-2 rounded-full bg-green-400 animate-pulse" />}
      ...
      {fieldAlerts.length > 0 && (
        <div className="px-3 py-2 border-b border-border/30">
          {fieldAlerts.slice(0, 2).map(a => (
            <div key={a.id} className={cn('font-cond text-[10px] font-bold flex items-center gap-1.5',
...
function GameRow({ game }: { game: any }) {
  ...
  async function updateStatus(newStatus: string) {
    ...
    await sb.from('ops_log').insert({
      event_id: 1, message: `Game #${game.id} status → ${newStatus} (via Command Center)`,
      log_type: newStatus === 'Live' ? 'ok' : newStatus === 'Final' ? 'info' : 'warn',
      source: 'command_center', occurred_at: new Date().toISOString(),
    })
    setManaging(false)
  ...
  return (
    <div>
      <button onClick={() => setManaging(m => !m)} className="w-full flex items-center justify-between px-4 py-2.5 hover:bg-white/5 transition-colors text-left">
        <div>
          <div className="font-cond text-[12px] font-bold text-white">
            {game.home_team?.name ?? '?'} vs {game.away_team?.name ?? '?'}
          </div>
          <div className="font-cond text-[10px] text-muted">{game.scheduled_time} · #{game.id}</div>
        </div>
        <div className="flex items-center gap-2">
          <span className={statusCfg[game.status] ?? 'badge-scheduled'}>{game.status?.toUpperCase()}</span>
          <ArrowUpRight size={12} className={cn('text-muted transition-transform', managing && 'rotate-45')} />
        </div>
      </button>
      ...
      {managing && (
        <div className="px-4 pb-3 bg-black/20">
          <div className="font-cond text-[9px] font-bold tracking-widest text-muted uppercase mb-2">UPDATE STATUS</div>
          <div className="flex flex-wrap gap-1.5">
            {['Scheduled','Starting','Live','Halftime','Final','Delayed','Suspended'].map(s => (
              <button key={s} onClick={() => updateStatus(s)}
                className={cn('font-cond text-[10px] font-bold px-2.5 py-1.5 rounded border transition-colors',
                  game.status === s
...
function HandoffHistory() {
  ...
  useEffect(() => {
    const sb = createClient()
    sb.from('shift_handoffs').select('*').eq('event_id', 1)
  ...
  return (
    <div className="grid grid-cols-3 gap-4">
      <div className="space-y-2">
        <div className="font-cond text-[10px] font-bold tracking-widest text-muted uppercase mb-2">PREVIOUS HANDOFFS</div>
        {history.map(h => (
          <button key={h.id} onClick={() => setSelected(h)}
            className={cn('w-full text-left px-3 py-2.5 rounded-lg border transition-colors',
              selected?.id === h.id ? 'bg-navy border-blue-400 text-white' : 'bg-surface-card border-border text-muted hover:text-white'
            )}>
      ...
      <div className="col-span-2">
        {selected ? (
          <div className="bg-surface-card border border-border rounded-xl p-5">
            <pre className="font-mono text-[11px] text-white whitespace-pre-wrap leading-relaxed">{selected.summary}</pre>
          </div>
        ) : (
