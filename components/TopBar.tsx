...
export function TopBar({ tabs, activeTab, onTabChange, userRole, onSignOut, isAdmin, onChangeEvent }: Props) {
  ...
  return (
    <header className="flex items-stretch flex-shrink-0"
      ...
      {/* Wordmark */}
      <div className="flex items-center gap-2.5 pl-4 pr-5 flex-shrink-0"
        ...
        <span className="font-cond text-[18px] font-black tracking-[0.15em] text-white ml-1">LEAGUEOPS</span>
      </div>
            ...
            <div key={group.label} className="relative flex items-stretch">
              ...
              >
                ...
                <div className="flex flex-col items-start leading-none">
                  <span className="font-cond text-[11px] font-black tracking-[0.12em]"
                    style={{ color: labelColor }}>
                    {group.label}
                  </span>
                  {/* Show which sub-item is active */}
                  {subLabel && (
                    <span className="font-cond text-[9px] tracking-wide mt-0.5"
                      style={{ color: '#3a5070' }}>
                      {subLabel}
                    </span>
                  )}
                ...
                {!isDirect && (
                  <ChevronDown size={10} className="flex-shrink-0 transition-transform"
                    style={{ color: labelColor, transform: isOpen ? 'rotate(180deg)' : 'none', marginTop: subLabel ? '-4px' : undefined }} />
                )}
              </button>

              {/* Dropdown */}
              {!isDirect && isOpen && (
                <div className="absolute top-full left-0 z-50 py-1 rounded-b-xl shadow-2xl min-w-[160px]"
                  style={{ background: '#061428', border: '1px solid #1a2d50', borderTop: 'none' }}>
                  {group.items!.map(item => (
                    <button key={item.id}
                      onClick={() => { onTabChange(item.id); setOpenGroup(null) }}
      ...
      <div className="flex items-center gap-4 px-5 flex-shrink-0"
        ...
        {userRole && (
          <div className="flex items-center gap-2.5 pl-4" style={{ borderLeft: '1px solid #1a2d50' }}>
            <span className={cn('font-cond text-[9px] font-black tracking-[.12em] px-2 py-1 rounded',
