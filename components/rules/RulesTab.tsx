...
> = {
  lightning: {
    ...
    icon: <Zap size={14} />,
    color: 'text-red-400',
    desc: 'Lightning delay radius, hold duration, and auto-detection',
  },
  heat: {
    label: 'Heat',
    icon: <Thermometer size={14} />,
    color: 'text-orange-400',
    desc: 'Heat index thresholds and mandatory break intervals',
  },
  wind: {
    label: 'Wind',
    icon: <Wind size={14} />,
    color: 'text-yellow-400',
    desc: 'Wind advisory and suspension thresholds',
  },
  weather: {
    label: 'Weather',
    icon: <Clock size={14} />,
    color: 'text-blue-300',
    desc: 'Polling interval, caching, and weather provider settings',
  },
  referee: {
    label: 'Referee',
    icon: <Users size={14} />,
    color: 'text-green-400',
    desc: 'Travel buffers, max games, grade requirements per division',
  },
  scheduling: {
    label: 'Scheduling',
    icon: <Calendar size={14} />,
    color: 'text-purple-400',
    desc: 'Game duration, rest windows, start/end time constraints',
  },
  general: {
    label: 'General',
    icon: <Settings size={14} />,
    color: 'text-muted',
    desc: 'Tournament name, location, and app-level settings',
  },
}
        ...
        <div className="space-y-4">
          ...
          <div className="bg-surface-card border border-border rounded-lg p-4">
            ...
            <div className="space-y-2">
              ...
              <StatRow
                ...
                value={overrideCount}
                color={overrideCount > 0 ? 'text-yellow-400' : 'text-green-400'}
              />
              <StatRow
                label="Using Defaults"
                value={rules.length - overrideCount}
                color="text-green-400"
              />
              <StatRow
                label="Unsaved Changes"
                value={dirtyCount}
                color={dirtyCount > 0 ? 'text-orange-400' : 'text-muted'}
              />
            </div>

            {overrideCount > 0 && (
              <div className="mt-3 pt-3 border-t border-border">
                <div className="font-cond text-[10px] font-bold text-muted uppercase mb-2">
                  ACTIVE OVERRIDES
                </div>
                  ...
                  {rules
                    ...
                    .map((r) => (
                      <div key={r.id} className="flex justify-between text-[10px]">
                        <span className="font-cond text-yellow-400 truncate flex-1 mr-2">
                          {r.rule_label}
                        </span>
          ...
          <div className="bg-surface-card border border-border rounded-lg p-4">
            <div className="font-cond text-[10px] font-black tracking-widest text-muted uppercase mb-3 flex justify-between">
              ...
              <button onClick={loadChanges} className="text-blue-300 hover:text-white">
                <RotateCcw size={11} />
              </button>
            </div>
            {changes.length === 0 ? (
              <div className="text-[11px] text-muted font-cond text-center py-4">
                No changes yet
              </div>
            ) : (
                ...
                {changes.map((c) => (
                  <div key={c.id} className="text-[10px] border-b border-border/40 pb-2">
                    ...
                    <div className="text-muted mt-0.5 font-cond">
                      {c.changed_by} ·{' '}
                      {new Date(c.changed_at).toLocaleTimeString('en-US', {
                        hour: 'numeric',
                        minute: '2-digit',
                      })}
                    </div>
          ...
          <div className="bg-surface-card border border-border rounded-lg p-4">
            ...
            <div className="space-y-2">
              {[
                ...
                {
                  label: 'Shorter games (50 min)',
                  key: 'scheduling.game_duration_min',
                  value: '50',
                },
              ].map((preset) => (
                <button
                  ...
                  onClick={async () => {
                    ...
                    if (!rule) {
                      toast.error(`Rule ${preset.key} not found`)
                      return
                    }
  ...
  return (
    <div
      className={cn(
        ...
        isDirty ? 'bg-orange-900/10' : rule.is_override ? 'bg-yellow-900/8' : 'hover:bg-white/5'
      )}
    >
      {/* Label + description */}
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-0.5">
          ...
          {isDirty && (
            <span className="font-cond text-[9px] font-bold text-orange-400 bg-orange-900/30 px-1.5 py-0.5 rounded">
              UNSAVED
            </span>
          )}
          {rule.description && (
        ...
        {showDesc && rule.description && (
          <div className="text-[10px] text-muted font-sans mb-1 leading-relaxed">
            {rule.description}
          </div>
        )}
        {!isDefault && !isDirty && (
        ...
        {rule.unit && <div className="font-cond text-[9px] text-muted/60 mt-0.5">{rule.unit}</div>}
      </div>
        ...
        {rule.value_type === 'boolean' ? (
          <button
            ...
            className={cn(
              ...
              editValue === 'true' ? 'bg-green-600 border-green-500' : 'bg-gray-700 border-gray-600'
            )}
          >
            <span
              className={cn(
