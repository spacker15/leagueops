...
export function IncidentsTab() {
  ...
  // Mobile tab toggle
  const [mobileTab, setMobileTab] = useState<'form' | 'log'>('form')
    ...
    <div>
      ...
      <div className="flex lg:hidden border-b border-border mb-4">
        <button
          ...
          className={cn(
            'flex-1 py-2.5 font-cond text-[12px] font-black tracking-widest uppercase border-b-2 transition-colors',
            mobileTab === 'form' ? 'border-red text-white' : 'border-transparent text-muted'
          )}
        >
          LOG INCIDENT
        </button>
        <button
          ...
          className={cn(
            'flex-1 py-2.5 font-cond text-[12px] font-black tracking-widest uppercase border-b-2 transition-colors',
            mobileTab === 'log' ? 'border-red text-white' : 'border-transparent text-muted'
          )}
        >
          LOG ({state.incidents.length})
        </button>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* LEFT: Forms */}
        <div className={cn(mobileTab === 'log' ? 'hidden lg:block' : 'block')}>
          {/* ── Incident Form ── */}
            ...
            <div className="grid grid-cols-2 gap-3 mb-3">
              <FormField label="Field">
                <select
                  className={sel}
                  value={trField}
                  onChange={(e) => setTrField(e.target.value)}
                >
        ...
        {/* RIGHT: Incident log */}
        <div className={cn(mobileTab === 'form' ? 'hidden lg:block' : 'block')}>
