...
export function ProgramLeaderDashboard() {
  ...
  const inp =
    'w-full bg-surface-card border border-border text-white px-3 py-2 rounded-lg text-[13px] outline-none'
    ...
    <div className="min-h-screen bg-surface p-4">
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        {teamRegs.map((team) => {
          ...
          return (
            <div key={team.id} className="bg-surface-card border border-border rounded-xl p-5">
              {/* Team header */}
              <div className="flex items-center justify-between mb-3">
                <div>
                  ...
                  <div className="font-cond text-[11px] text-muted uppercase">{team.division}</div>
                </div>
              ...
              {/* Coaches section */}
              <div className="mb-3">
                <div className="font-cond text-[10px] font-black tracking-[.12em] text-muted uppercase mb-2 flex items-center gap-2">
                  COACHES
                ...
                {coaches.length === 0 ? (
                  <div className="text-[12px] text-muted italic">No coaches assigned yet.</div>
                ) : (
                    ...
                    {coaches.map((ct) => (
                      <div key={ct.id} className="flex items-center gap-2 flex-wrap">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="text-[11px] text-muted">{ct.coaches?.email ?? ''}</div>
                        </div>
              ...
              <div>
                <div className="font-cond text-[10px] font-black tracking-[.12em] text-muted uppercase mb-1">
                  AVAILABLE DATES
                </div>
                    ...
                    <div className="space-y-1.5">
                      {teamGames.map((game) => {
                        ...
                        return (
                          <div
                            key={game.id}
                            className={`flex items-center justify-between gap-2 rounded-lg px-2.5 py-2 bg-surface-card border border-border ${isCancelled ? 'opacity-50' : ''}`}
                          >
        ...
        {teamRegs.length === 0 && (
          <div className="text-center text-muted text-[13px] py-8">No teams registered yet.</div>
        )}
      </div>
