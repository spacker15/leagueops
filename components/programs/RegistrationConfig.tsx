...
interface Division {
  id: number; name: string; age_group: string | null; gender: string | null
  sort_order: number; is_active: boolean; max_teams: number | null
}
...
export function RegistrationConfig() {
  ...
  const load = useCallback(async () => {
    ...
    const [{ data: divs }, { data: qs }] = await Promise.all([
      ...
      sb.from('registration_questions').select('*').eq('event_id', 1).order('section').order('sort_order'),
    ])
  ...
  async function addDivision() {
    if (!newDivName) { toast.error('Division name required'); return }
    const sb = createClient()
    const nextOrder = Math.max(0, ...divisions.map(d => d.sort_order)) + 1
    const { error } = await sb.from('registration_divisions').insert({
      event_id: 1, name: newDivName,
      age_group: newDivAge || null, gender: newDivGender || null,
    ...
    setNewDivName(''); setNewDivAge(''); setNewMaxTeams('')
    load()
  }
  ...
  async function deleteDivision(id: number, name: string) {
    if (!confirm(`Remove division "${name}"? This will not affect already-registered teams.`)) return
    const sb = createClient()
  ...
  async function addQuestion() {
    if (!newQText) { toast.error('Question text required'); return }
    const key = newQKey || textToKey(newQText)
    const sb  = createClient()
    const nextOrder = Math.max(0, ...questions.filter(q => q.section === newQSection).map(q => q.sort_order)) + 1
    const options = newQType === 'select' && newQOptions
      ? newQOptions.split(',').map(o => o.trim()).filter(Boolean)
      : null
    const { error } = await sb.from('registration_questions').insert({
      event_id: 1, section: newQSection, question_key: key,
      question_text: newQText, question_type: newQType,
      placeholder: newQPlaceholder || null, options,
      is_required: newQRequired, is_active: true, sort_order: nextOrder,
    })
    if (error) { toast.error(error.message); return }
    toast.success('Question added')
    setNewQText(''); setNewQKey(''); setNewQPlaceholder(''); setNewQOptions('')
  ...
  return (
    <div>
      ...
      <div className="text-[11px] text-muted font-cond mb-4 leading-relaxed">
        Configure what appears on the public registration form. Changes take effect immediately for new registrations.
      </div>
        ...
        {(['divisions','questions'] as const).map(t => (
          <button key={t} onClick={() => setActiveTab(t)}
            className={cn('font-cond text-[12px] font-bold px-4 py-2 rounded-lg border transition-colors',
              activeTab === t ? 'bg-navy border-blue-400 text-white' : 'bg-surface-card border-border text-muted hover:text-white'
            )}>
            {t === 'divisions' ? `Divisions (${divisions.length})` : `Questions (${questions.length})`}
          </button>
        ))}
      </div>
        ...
        <>
          ...
          {activeTab === 'divisions' && (
            <div className="grid grid-cols-2 gap-6">
              ...
              <div className="bg-surface-card border border-border rounded-xl p-4">
                <div className="font-cond font-black text-[13px] tracking-wide mb-4">ADD DIVISION</div>
                <div className="space-y-3">
                  <div>
                    <label className="font-cond text-[10px] font-bold tracking-widest text-muted uppercase block mb-1">Division Name *</label>
                    <input className="w-full bg-surface border border-border text-white px-2.5 py-2 rounded text-[13px] outline-none focus:border-blue-400"
                      value={newDivName} onChange={e => setNewDivName(e.target.value)} placeholder="e.g. 14U Boys" />
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="font-cond text-[10px] font-bold tracking-widest text-muted uppercase block mb-1">Age Group</label>
                      <input className="w-full bg-surface border border-border text-white px-2.5 py-2 rounded text-[13px] outline-none focus:border-blue-400"
                        value={newDivAge} onChange={e => setNewDivAge(e.target.value)} placeholder="e.g. 14U" />
                    </div>
                    <div>
                      <label className="font-cond text-[10px] font-bold tracking-widest text-muted uppercase block mb-1">Gender</label>
                      <select className="w-full bg-surface border border-border text-white px-2.5 py-2 rounded text-[13px] outline-none focus:border-blue-400"
                        value={newDivGender} onChange={e => setNewDivGender(e.target.value)}>
                        <option>Boys</option><option>Girls</option><option>Co-Ed</option><option>Open</option>
                      </select>
                    </div>
                  </div>
                  <div>
                    <label className="font-cond text-[10px] font-bold tracking-widest text-muted uppercase block mb-1">Max Teams (blank = unlimited)</label>
                    <input type="number" className="w-full bg-surface border border-border text-white px-2.5 py-2 rounded text-[13px] outline-none focus:border-blue-400"
                      value={newMaxTeams} onChange={e => setNewMaxTeams(e.target.value)} placeholder="Leave blank for unlimited" min="1" />
                  </div>
                  ...
                  {divisions.map(div => (
                    <div key={div.id} className={cn(
                      ...
                      <input
                        ...
                        onBlur={e => { if (e.target.value !== String(div.max_teams ?? '')) updateDivMaxTeams(div.id, e.target.value) }}
                        className="bg-surface border border-border text-white px-2 py-1 rounded text-[11px] w-16 outline-none focus:border-blue-400"
                        title="Max teams"
                      />
                      <button onClick={() => toggleDivision(div.id, div.is_active)}
          ...
          {activeTab === 'questions' && (
            <div className="grid grid-cols-2 gap-6">
              {/* Add form */}
              <div className="bg-surface-card border border-border rounded-xl p-4">
                <div className="font-cond font-black text-[13px] tracking-wide mb-4">ADD QUESTION</div>
                <div className="space-y-3">
                  <div>
                    <label className="font-cond text-[10px] font-bold tracking-widest text-muted uppercase block mb-1">Appears On</label>
                    <select className="w-full bg-surface border border-border text-white px-2.5 py-2 rounded text-[13px] outline-none focus:border-blue-400"
                      value={newQSection} onChange={e => setNewQSection(e.target.value)}>
                      {SECTIONS.map(s => <option key={s} value={s}>{SECTION_LABELS[s]}</option>)}
                    </select>
                  </div>
                  <div>
                    <label className="font-cond text-[10px] font-bold tracking-widest text-muted uppercase block mb-1">Question Text *</label>
                    <input className="w-full bg-surface border border-border text-white px-2.5 py-2 rounded text-[13px] outline-none focus:border-blue-400"
                      value={newQText} onChange={e => { setNewQText(e.target.value); setNewQKey(textToKey(e.target.value)) }}
                      placeholder="e.g. Jersey color" />
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="font-cond text-[10px] font-bold tracking-widest text-muted uppercase block mb-1">Field Type</label>
                      <select className="w-full bg-surface border border-border text-white px-2.5 py-2 rounded text-[13px] outline-none focus:border-blue-400"
                        value={newQType} onChange={e => setNewQType(e.target.value)}>
                        {QUESTION_TYPES.map(t => <option key={t} value={t}>{t}</option>)}
                      </select>
                    </div>
                    <div>
                      <label className="font-cond text-[10px] font-bold tracking-widest text-muted uppercase block mb-1">Placeholder</label>
                      <input className="w-full bg-surface border border-border text-white px-2.5 py-2 rounded text-[13px] outline-none focus:border-blue-400"
                        value={newQPlaceholder} onChange={e => setNewQPlaceholder(e.target.value)} placeholder="e.g. Navy Blue" />
                    </div>
                  </div>
                  {newQType === 'select' && (
                    <div>
                      <label className="font-cond text-[10px] font-bold tracking-widest text-muted uppercase block mb-1">Options (comma-separated)</label>
                      <input className="w-full bg-surface border border-border text-white px-2.5 py-2 rounded text-[13px] outline-none focus:border-blue-400"
                        value={newQOptions} onChange={e => setNewQOptions(e.target.value)} placeholder="Option 1, Option 2, Option 3" />
                    </div>
                  )}
                  <div className="flex items-center gap-2">
                    <button onClick={() => setNewQRequired(r => !r)}
                      className={cn('relative w-9 h-5 rounded-full border-2 transition-all',
                        newQRequired ? 'bg-green-600 border-green-500' : 'bg-gray-700 border-gray-600'
                      )}>
                          ...
                          <div key={q.id} className={cn(
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2 mb-0.5">
                                <span className="font-cond font-bold text-[12px] text-white">{q.question_text}</span>
                                {q.is_required && (
                                  <span className="font-cond text-[9px] font-bold text-red-400 bg-red-900/30 px-1.5 py-0.5 rounded">REQUIRED</span>
                                )}
                              </div>
                              <div className="flex items-center gap-2 text-[10px]">
                                <span className={cn('font-cond font-bold px-1.5 py-0.5 rounded', SECTION_COLORS[q.section])}>
                                  {q.section}
                                </span>
                                <span className="text-muted">{q.question_type}</span>
                                {q.options && <span className="text-muted">{q.options.length} options</span>}
                              </div>
                            </div>
                            <div className="flex items-center gap-1.5 flex-shrink-0">
                              <button onClick={() => toggleRequired(q.id, q.is_required)}
                                title="Toggle required"
                                className={cn('font-cond text-[9px] font-bold px-1.5 py-1 rounded border transition-colors',
                                  q.is_required
                              ...
                              <button onClick={() => deleteQuestion(q.id, q.question_text)}
                                className="text-muted hover:text-red-400 transition-colors">
                                <Trash2 size={12} />
                              </button>
                            </div>
