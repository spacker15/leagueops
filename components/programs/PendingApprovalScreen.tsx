...
export function PendingApprovalScreen() {
  ...
  async function loadStatus() {
    ...
    const [{ data: prog }, { data: teamData }] = await Promise.all([
      ...
      sb.from('team_registrations').select('id, team_name, division, status').eq('program_id', userRole.program_id),
    ])
        ...
        <div className="w-full max-w-lg">
          ...
          <div className="text-center mb-8">
            <div className={cn(
              ...
              {program?.logo_url ? (
                <img src={program.logo_url} alt="" className="w-14 h-14 object-contain rounded-full" />
              ) : isRejected ? (
            ...
            <div className={cn('font-cond font-black text-[26px] tracking-wide mb-2',
            ...
            <div className="font-cond text-[12px] text-muted">{program?.contact_name} · {program?.contact_email}</div>
          </div>
            ...
            {isRejected ? (
              ...
              <div>
                ...
                <div className="font-cond text-[13px] text-gray-300 leading-relaxed">
                  The league administrator will review your program and team registrations.
                  Once approved you'll have full access to manage your teams and rosters.
                  This typically takes <span className="text-white font-bold">1–2 business days</span>.
                </div>
              </div>
            )}
          </div>
                ...
                {teams.map(t => (
                  <div key={t.id} className="flex items-center justify-between text-[12px]">
                    ...
                    <span className={cn('font-cond text-[10px] font-black px-2 py-0.5 rounded tracking-wider',
                      ...
                      t.status === 'rejected'  ? 'bg-red-900/40 text-red-400' :
                      t.status === 'waitlist'  ? 'bg-orange-900/40 text-orange-400' :
          ...
          <div className="text-center mt-6 font-cond text-[10px] text-muted">
            Submitted {new Date(program?.created_at ?? '').toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}
          </div>
        </div>
      </div>
    </div>
