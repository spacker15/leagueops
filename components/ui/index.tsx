...
export function Btn({ variant = 'primary', size = 'md', className, children, ...props }: BtnProps) {
  const v = {
    ...
    outline: 'bg-transparent border border-border text-muted hover:text-white hover:border-blue-400/60',
  }[variant]
  const s = {
  ...
  return (
    <button className={cn('font-cond font-bold tracking-wide uppercase rounded transition-colors disabled:opacity-40 disabled:cursor-not-allowed', v, s, className)} {...props}>
      {children}
    </button>
  )
}
...
export function FormField({ label, children, className }: { label: string; children: React.ReactNode; className?: string }) {
  return (
    <div className={cn('flex flex-col gap-1', className)}>
      <label className="font-cond text-[9px] font-bold tracking-widest text-muted uppercase">{label}</label>
      {children}
    </div>
  )
}

const base = 'bg-[#040e24] border border-[#1e3060] text-white px-2.5 py-1.5 rounded-lg text-[12px] outline-none focus:border-blue-400/60 transition-colors'

export function Input(props: React.InputHTMLAttributes<HTMLInputElement>) {
  return <input className={cn(base, props.className)} {...props} />
}
export function Select({ children, ...props }: React.SelectHTMLAttributes<HTMLSelectElement>) {
  return <select className={cn(base, 'cursor-pointer', props.className)} {...props}>{children}</select>
}
...
export function Card({ children, className }: { children: React.ReactNode; className?: string }) {
  return <div className={cn('bg-surface-card border border-[#1e3060] rounded-xl', className)}>{children}</div>
}

// ─── SectionHeader ────────────────────────────────────────────
export function SectionHeader({ children, className }: { children: React.ReactNode; className?: string }) {
  return (
    <div className={cn('font-cond text-[10px] font-black tracking-widest text-muted uppercase border-b border-[#1e3060] pb-2 mb-3', className)}>
      {children}
    </div>
  )
}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 backdrop-blur-sm"
      ...
      <div className="bg-[#060f2a] border border-[#1e3060] rounded-xl w-[580px] max-w-[95vw] max-h-[88vh] overflow-y-auto shadow-2xl">
        ...
        {footer && <div className="flex justify-end gap-2 px-5 py-3 border-t border-[#1e3060]">{footer}</div>}
      </div>
    </div>
  )
}
  ...
  return (
    <div className="mb-2">
      <div className="flex justify-between items-center mb-1">
        ...
        <span className="font-mono text-[11px]" style={{ color }}>{value}/{total}</span>
      </div>
...
export function Avatar({ name, variant = 'blue' }: { name: string; variant?: 'blue' | 'red' | 'green' }) {
  ...
  const cls  = { blue: 'bg-blue-900/30 text-blue-300', red: 'bg-red-900/20 text-red-400', green: 'bg-green-900/20 text-green-400' }[variant]
  return <div className={cn('w-8 h-8 rounded-full flex items-center justify-center font-cond text-[12px] font-black flex-shrink-0', cls)}>{init}</div>
}

// ─── Pill ─────────────────────────────────────────────────────
export function Pill({ children, variant = 'blue' }: { children: React.ReactNode; variant?: 'blue' | 'green' | 'red' | 'yellow' | 'gray' }) {
  const cls = { blue: 'bg-blue-900/25 text-blue-300', green: 'bg-green-900/25 text-green-400', red: 'bg-red-900/20 text-red-400', yellow: 'bg-yellow-900/20 text-yellow-400', gray: 'bg-gray-700/30 text-gray-400' }[variant]
  return <span className={cn('font-cond text-[10px] font-bold tracking-wide px-2 py-0.5 rounded', cls)}>{children}</span>
}
