...
import {
  Printer, CheckCircle, Search, XCircle, Clock, ShieldCheck, RefreshCw,
} from 'lucide-react'
...
interface PlayerWithExtras {
  ...
  home_division?: string | null; team_id?: number
  team?: { id: number; name: string; division: string }
  token?: string
}

interface PendingApproval {
  id: number; player_id: number; game_id: number
  first_game_id: number | null; opposing_team_name: string
...
interface CheckinState {
  ...
  approvalId?: number; message?: string; opposingTeam?: string
}
...
async function ensureTokens(playerIds: number[]): Promise<Record<number, string>> {
  ...
  await Promise.all(
    playerIds.map(id =>
      sb.from('player_qr_tokens').upsert({ player_id: id, event_id: 1 }, { onConflict: 'player_id,event_id' })
    )
  )
  // Fetch back tokens
  const { data } = await sb
...
export function CheckInTab() {
  ...
  useEffect(() => {
    ...
    // Load league info
    const sb = createClient()
    sb.from('events').select('name, logo_url').eq('id', 1).single()
      .then(({ data }) => {
        if (data) {
  ...
  async function loadPlayers(teamId: number): Promise<PlayerWithExtras[]> {
    ...
    const { data } = await sb
      .from('players')
      .select('id, name, number, position, usa_lacrosse_number, home_division, team_id, team:teams(id, name, division)')
      .eq('team_id', teamId)
      .order('name')
  ...
  async function loadGameCheckins(gameId: number) {
    ...
    setCheckinStates(prev => {
      const next = { ...prev }
      for (const id of ids) if (!next[id] || next[id].status === 'unchecked') next[id] = { status: 'checked' }
      return next
    })
  }
  ...
  async function loadPendingForGame(gameId: number) {
    ...
    setCheckinStates(prev => {
      ...
      for (const a of data) {
        next[a.player_id] = {
          status: 'pending_approval', approvalId: a.id,
  ...
  useEffect(() => { if (tab === 'approvals') loadAllApprovals() }, [tab, loadAllApprovals])
  ...
  async function togglePlayer(player: PlayerWithExtras) {
    ...
    if (result.eligible === true) {
      ...
      setCheckinStates(prev => ({ ...prev, [player.id]: { status: 'blocked_play_down', message: result.message } }))
  ...
  function printCards(players: PlayerWithExtras[]) {
    ...
    const logoHtml = leagueLogo
      ...
      : `<div class="logo-text">${leagueName.split(' ').map(w => w[0]).join('').slice(0,3)}</div>`
  ...
  const filteredHome = cardSearch
    ? homePlayers.filter(p => {
        ...
        return p.name.toLowerCase().includes(q) || String(p.number ?? '').includes(q) || (p.usa_lacrosse_number ?? '').includes(q)
      })
    : homePlayers
      ...
      <div className="flex items-center gap-0 mb-4 border-b border-border">
        {[
          ...
          <button key={t.id} onClick={() => setTab(t.id as Tab)}
            className={cn('font-cond font-bold text-[12px] tracking-widest uppercase px-4 py-2 border-b-2 transition-colors',
            ...
            {selectedGameId && totalPlayers > 0 && (
              <div className="flex items-center gap-3">
                <div className={cn('font-mono text-xl font-bold',
                  checkedCount === totalPlayers ? 'text-green-400' : checkedCount > 0 ? 'text-yellow-400' : 'text-muted'
          ...
          {pendingApprovals.length > 0 && (
            <div className="bg-yellow-900/15 border border-yellow-800/40 rounded-xl p-4 mb-4 space-y-2">
              <div className="font-cond font-black text-[12px] text-yellow-400 flex items-center gap-2 mb-2">
                <Clock size={13} /> AWAITING APPROVAL — as <span className="text-white ml-1">{approverName}</span>
              </div>
              ...
              {viewMode === 'list' && (
                <div className="grid grid-cols-2 gap-4">
                  <RosterList label={selectedGame.home_team?.name ?? 'Home'} players={homePlayers}
                    states={checkinStates} onToggle={togglePlayer} onPrintTeam={() => printCards(homePlayers)} />
                  <RosterList label={selectedGame.away_team?.name ?? 'Away'} players={awayPlayers}
                    states={checkinStates} onToggle={togglePlayer} onPrintTeam={() => printCards(awayPlayers)} />
                </div>
              )}
                  ...
                  {[
                    ...
                    <div key={label} className="mb-6">
                      <div className="flex items-center gap-3 mb-3">
                        <div className="font-cond font-black text-[14px] tracking-wide text-white">{label}</div>
                        <div className={cn('font-cond text-[12px] font-bold',
                          players.filter(p => checkinStates[p.id]?.status === 'checked').length === players.length && players.length > 0
                      ...
                      <div className="grid grid-cols-[repeat(auto-fill,minmax(175px,1fr))] gap-3">
                        {players.map(p => (
                          <PlayerCard key={p.id} player={p} state={checkinStates[p.id] ?? { status: 'unchecked' }}
                            baseUrl={baseUrl} leagueLogo={leagueLogo}
                            onToggle={() => togglePlayer(p)} onPrint={() => printCards([p])} />
                        ))}
                      </div>
                    </div>
                  ))}
...
// ─── Roster list panel ────────────────────────────────────────
function RosterList({ label, players, states, onToggle, onPrintTeam }: {
      ...
      <div className="bg-navy/60 px-4 py-2.5 border-b border-border flex justify-between items-center">
        ...
        <div className="flex items-center gap-2">
          <div className={cn('font-mono text-[18px] font-bold',
            checked === players.length && players.length > 0 ? 'text-green-400' : checked > 0 ? 'text-yellow-400' : 'text-muted'
          )}>{checked}/{players.length}</div>
          <button onClick={onPrintTeam} title="Print team cards"
          ...
          {players.map(p => {
            ...
            return (
              <button key={p.id} onClick={() => onToggle(p)}
                ...
                <div className={cn('w-9 h-9 rounded-full flex items-center justify-center font-cond font-black text-[14px] flex-shrink-0',
                ...
                <div className="flex-1 min-w-0">
                  <div className={cn('font-cond font-bold text-[14px] leading-tight',
                  ...
                  {s.status === 'pending_approval'  && <div className="font-cond text-[10px] text-yellow-400">⏳ {s.message}</div>}
                  {s.status === 'blocked_play_down' && <div className="font-cond text-[10px] text-red-400">⛔ {s.message}</div>}
                  {p.usa_lacrosse_number && s.status !== 'blocked_play_down' && (
                    <div className="font-mono text-[10px] text-muted">USA #{p.usa_lacrosse_number}</div>
                  )}
                </div>
                <div className="flex-shrink-0">
                  {s.status === 'checked'           && <CheckCircle size={18} className="text-green-400" />}
                  {s.status === 'pending_approval'  && <Clock size={18} className="text-yellow-400" />}
                  {s.status === 'blocked_play_down' && <XCircle size={18} className="text-red-400" />}
                  {s.status === 'unchecked'         && <div className="w-5 h-5 rounded-full border-2 border-border" />}
                </div>
              </button>
            )
          })}
...
function PlayerCard({ player, state, baseUrl, leagueLogo, onToggle, onPrint }: {
  player: PlayerWithExtras; state: CheckinState
  baseUrl: string; leagueLogo: string | null
  ...
  return (
    <div className={cn(
      ...
      <div className={cn('px-3 py-2.5 border-b border-border/50',
        isChecked ? 'bg-green-900/30' : 'bg-navy'
      )}>
        <div className="flex items-center gap-2.5">
          {/* League logo */}
          {leagueLogo ? (
            <img src={leagueLogo} alt="League" className="w-8 h-8 object-contain rounded flex-shrink-0 bg-white/10" />
          ) : (
          ...
          <div className={cn(
            ...
            isChecked ? 'bg-green-700 text-white border-green-500/40' :
            isPending ? 'bg-yellow-700 text-white border-yellow-500/40' :
            isBlocked ? 'bg-red-900/60 text-red-400 border-red-800/40' :
            'bg-red text-white border-red-700/40'
          )}>{player.number ?? '?'}</div>
          {/* Name */}
          <div className="min-w-0 flex-1">
            <div className={cn('font-cond font-black text-[14px] leading-tight truncate',
              isChecked ? 'text-green-300' : isPending ? 'text-yellow-300' : isBlocked ? 'text-red-400' : 'text-white'
            )}>{player.name}</div>
            <div className="font-cond text-[10px] text-muted truncate">{team?.name ?? '—'} · {team?.division ?? '—'}</div>
          </div>
          {/* Status icon */}
      ...
      <div className="p-2.5">
        {qrUrl ? (
          ...
          <div className="w-full aspect-square rounded-lg border-2 border-dashed border-border flex flex-col items-center justify-center gap-2 bg-white/5">
            <div className="font-cond text-[10px] text-muted text-center">QR auto-generates when player loads</div>
          </div>
        )}
      </div>
      ...
      <div className="px-3 pb-2 space-y-1.5">
        {player.usa_lacrosse_number && (
          <div className="flex justify-between items-center">
            <span className="font-cond text-[10px] font-bold tracking-widest text-muted uppercase">USA Lax #</span>
            <span className="font-mono text-[13px] font-bold text-white">{player.usa_lacrosse_number}</span>
          </div>
        )}
        <div className="flex justify-between items-center">
          <span className="font-cond text-[10px] font-bold tracking-widest text-muted uppercase">Jersey</span>
          <span className="font-mono text-[14px] font-bold text-white">#{player.number ?? '—'}</span>
        </div>
      </div>
      ...
      <div className="px-2.5 pb-2.5 flex gap-1.5">
        <button onClick={onToggle}
          className={cn('flex-1 font-cond text-[11px] font-bold py-2 rounded transition-colors',
          ...
          {isChecked ? '✓ CHECKED IN' : isPending ? '⏳ PENDING' : isBlocked ? '⛔ BLOCKED' : 'CHECK IN'}
        </button>
...
function ApprovalRow({ approval, applying, approverName, onApprove, onDeny }: {
  ...
  return (
    <div className="bg-black/20 border border-yellow-800/30 rounded-lg p-3">
      ...
      <div className="font-cond text-[10px] text-muted mb-2">Approving as: <span className="text-white">{approverName}</span></div>
      <div className="flex gap-2">
        <input className="flex-1 bg-surface border border-border text-white px-2 py-1.5 rounded text-[11px] outline-none focus:border-blue-400"
          placeholder="Coach name who approved (optional)" value={coachName} onChange={e => setCoachName(e.target.value)} />
  ...
  return (
    <div className="bg-surface-card border border-yellow-800/40 rounded-xl p-4">
      <div className="flex items-center gap-3 mb-3">
        ...
        <div className="font-cond text-[10px] text-muted">{new Date(approval.created_at).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' })}</div>
      </div>
      <div className="font-cond text-[11px] text-muted mb-3">
        Approving as: <span className="text-white font-bold">{approverName}</span>
      </div>
      <div className="flex gap-2">
        <input className="flex-1 bg-surface border border-border text-white px-2.5 py-2 rounded text-[12px] outline-none focus:border-blue-400"
          placeholder="Coach who verbally approved (optional)" value={coachName} onChange={e => setCoachName(e.target.value)} />
        <button onClick={() => onApprove(coachName)} disabled={applying}
