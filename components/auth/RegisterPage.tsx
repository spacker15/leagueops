...
function RegisterPageInner() {
  ...
  async function loadPrefill() {
    try {
      ...
      if (program) {
        ...
        prevTeams?.forEach((t: PreviousTeam) => {
          offers[t.id] = false
        })
  ...
  async function checkEmail(emailVal: string) {
    ...
    try {
      ...
      if (data.has_program) {
        setEmailConflict(
          `This email already has a program registered (${data.program_name}, ${data.program_status}). Please log in instead.`
        )
      }
  ...
  function togglePreviousTeam(teamId: number, accept: boolean) {
    ...
    if (accept) {
      ...
      setTeams((prev) =>
        prev.filter(
          (t) => !(t.name === pt.team_name && t.division === pt.division && t.fromPrevious)
        )
      )
    }
  }
  ...
  function validateProgram() {
    if (!progName || !contactName) {
      setError('Program name and contact name required')
      return false
    }
    setError('')
    return true
  ...
  function validateTeams() {
    for (let i = 0; i < teams.length; i++) {
      if (!teams[i].name) {
        setError(`Team ${i + 1} needs a name`)
        return false
      }
      if (!teams[i].division) {
        ...
        {mode === 'choose' && (
          <div className="grid grid-cols-2 gap-4">
            ...
            >
              <LogIn
                size={32}
                className="mx-auto mb-4 text-blue-300 group-hover:scale-110 transition-transform"
              />
              <div className="font-cond font-black text-[18px] text-white mb-2 tracking-wide">
                I HAVE AN ACCOUNT
              </div>
        ...
        {mode === 'login' && (
          <div className="bg-surface-card border border-border rounded-xl p-6 max-w-md mx-auto">
            ...
            <div className="text-center mt-4">
              ...
              <button
                onClick={() => setMode('register')}
                className="font-cond text-[11px] text-blue-300 hover:text-white"
              >
                Register here
              </button>
            </div>
          ...
          <>
            ...
            <div className="flex items-center mb-8">
              {(user
                ...
                const allSteps = user
                  ? ['program', 'teams', 'confirm']
                  : ['account', 'program', 'teams', 'confirm']
                    ...
                    <div
                      className={cn(
                        ...
                        mine < cur
                          ...
                          : mine === cur
                            ? 'bg-navy-light border-2 border-blue-400 text-white'
              ...
              <div className="bg-blue-900/20 border border-blue-800/40 rounded-xl p-4 mb-5 flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="font-cond text-[11px] text-muted">
                    Your program info is pre-filled. Update anything that's changed and add new
                    teams below.
                  </div>
                </div>
              </div>
          ...
          <div className="bg-surface-card border border-border rounded-xl p-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  type="email"
                  className={cn(inp, emailConflict ? 'border-red-500' : '')}
                  value={email}
                  onChange={(e) => {
            ...
            <div className="flex justify-between mt-5">
              ...
              <NavBtn
                dir="next"
                onClick={() => {
                  if (validateAccount()) setStep('program')
                }}
              />
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div className="bg-surface-card border border-border rounded-xl p-6">
              ...
              <div className="mb-5">
                ...
                <div className="flex items-start gap-4">
                  {logoPreview ? (
                    <div className="relative w-24 h-24 flex-shrink-0">
                      <img
                        src={logoPreview}
                        alt="Logo"
                        className="w-24 h-24 object-contain rounded-lg border border-border bg-white/5"
                      />
                      {!existingLogoUrl && (
                        <button
                          onClick={() => {
                  ...
                  <div className="text-[11px] text-muted font-cond leading-relaxed pt-1">
                    Your logo will display on all team pages.
                    <br />
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="col-span-2">
                  ...
                  <textarea
                    className={cn(inp, 'resize-y min-h-[60px]')}
                    value={notes}
            ...
            <div className="flex justify-between">
              {!user ? (
                ...
                <button
                  onClick={() => setMode('choose')}
                  className="flex items-center gap-2 font-cond text-[12px] text-muted hover:text-white"
                >
                  <ArrowLeft size={13} /> BACK
                </button>
              )}
              <NavBtn
                dir="next"
                onClick={() => {
                  if (validateProgram()) setStep('teams')
                }}
              />
            </div>
          </div>
          ...
          <div className="space-y-4">
                ...
                <div className="space-y-2">
                  {previousTeams.map((pt) => {
                    ...
                    return (
                      ...
                      >
                        <div>
                          <div className="font-cond font-bold text-[13px] text-white">
                            {pt.team_name}
                          </div>
                          <div className="font-cond text-[11px] text-blue-300">{pt.division}</div>
                          {pt.head_coach_name && (
                            <div className="font-cond text-[10px] text-muted">
                              Coach: {pt.head_coach_name}
                            </div>
                          )}
                        </div>
                        <div className="flex gap-2">
                          <button
                            onClick={() => togglePreviousTeam(pt.id, false)}
                            className={cn(
                          ...
                          <button
                            onClick={() => togglePreviousTeam(pt.id, true)}
                            className={cn(
                              'font-cond text-[11px] font-bold px-3 py-1.5 rounded border transition-colors',
                              accepted
                ...
                <div className="bg-navy/60 px-5 py-3 flex justify-between items-center border-b border-border">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="font-cond font-black text-[13px] tracking-wide text-blue-300">
                      TEAM {i + 1}{' '}
                      {team.fromPrevious && (
                        <span className="text-green-400 text-[10px] ml-1">↩ RETURNING</span>
                      )}
                    </span>
                    {team.name && (
                  ...
                  {teams.length > 1 && (
                    <button
                      ...
                      className="flex items-center gap-1 font-cond text-[10px] text-red-400 hover:text-red-300"
                    >
                  ...
                  {teamQs.length > 0 && (
                    <div className="grid grid-cols-2 gap-3">
                      {teamQs.map((q) => (
                        <div
                          key={q.id}
                          className={q.question_type === 'textarea' ? 'col-span-2' : ''}
                        >
                          <label className={lbl}>
                            {q.question_text}
                            {q.is_required && ' *'}
                          ...
                          <QInput
                            q={q}
                            value={team.customAnswers[q.question_key] ?? ''}
                            onChange={(v) => updateTeamAnswer(i, q.question_key, v)}
                            cls={inp}
                          />
                        </div>
                      ))}
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => updateTeam(i, 'coachPhone', e.target.value)}
                          placeholder="(555) 000-0000"
                        />
                      </div>
                    </div>
            ...
            <div className="flex justify-between">
              ...
              <NavBtn
                ...
                label="REVIEW"
                onClick={() => {
                  if (validateTeams()) setStep('confirm')
                }}
              />
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div className="bg-surface-card border border-border rounded-xl p-6">
              ...
              <div className="flex items-start gap-4 mb-5 pb-5 border-b border-border">
                {logoPreview ? (
                  <img
                    src={logoPreview}
                    alt=""
                    className="w-16 h-16 object-contain rounded-lg border border-border bg-white/5 flex-shrink-0"
                  />
                ) : (
                  <div className="w-16 h-16 rounded-lg border border-border bg-navy flex items-center justify-center flex-shrink-0">
                    <span className="font-cond font-black text-xl text-blue-300">
                      {progName
                        .split(' ')
                        .map((w) => w[0])
                        .join('')
                ...
                <div>
                  ...
                  {shortName && (
                    <div className="font-cond text-[12px] text-blue-300">{shortName}</div>
                  )}
                  <div className="font-cond text-[11px] text-muted mt-1">
                    {[city, state, association].filter(Boolean).join(' · ')}
                  </div>
                  <div className="font-cond text-[11px] text-muted">
                    {contactName} · {user?.email ?? email}
                  </div>
                </div>
              </div>
              ...
              <div className="mt-5 bg-yellow-900/20 border border-yellow-800/40 rounded-lg p-3">
                <div className="font-cond font-bold text-[11px] text-yellow-400 mb-1">
                  WHAT HAPPENS NEXT
                </div>
        ...
        {step === 'done' && (
          <div className="text-center py-8">
            ...
            <div className="font-cond text-[13px] text-muted mb-6 max-w-md mx-auto leading-relaxed">
              <span className="text-white font-bold">{progName}</span> — {teams.length} team
              {teams.length > 1 ? 's' : ''} submitted for review.
            </div>
            <a
              href="/"
...
function QInput({
  ...
  if (q.question_type === 'textarea')
    return (
      <textarea
        className={cn(cls, 'resize-y min-h-[70px]')}
        value={value}
...
function NavBtn({
  ...
  if (dir === 'back')
    return (
      <button
        onClick={onClick}
        className="flex items-center gap-2 font-cond font-black text-[13px] text-muted hover:text-white px-4 py-2.5 transition-colors"
      >
