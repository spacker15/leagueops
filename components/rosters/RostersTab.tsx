        ...
        <div>
          ...
          <div className="bg-surface-card border border-border rounded-md p-3 mb-3">
            ...
            <div className="space-y-1.5">
              <div>
                <div className="font-cond text-[10px] font-bold text-blue-300 mb-0.5">NFYLL / Full Format (10 columns)</div>
                <div className="font-mono text-[9px] text-muted bg-black/30 px-2 py-1 rounded">
                  USA Lacrosse #, Firstname, Lastname, Birthdate, Zipcode, Email, Div, Team, Association, Player or Coach
                </div>
              </div>
              <div>
                <div className="font-cond text-[10px] font-bold text-blue-300 mb-0.5">Simple Format (2–4 columns)</div>
                <div className="font-mono text-[9px] text-muted bg-black/30 px-2 py-1 rounded">
          ...
          {!pending ? (
            ...
            >
              ...
              <div className="font-cond font-bold text-muted text-sm tracking-wide">DROP CSV FILE HERE</div>
            ...
            <div className="bg-surface-card border border-border rounded-md overflow-hidden">
              <div className={`px-3 py-2 flex justify-between items-center border-b border-border ${committed ? 'bg-green-900/20' : 'bg-navy/60'}`}>
                ...
                <table className="text-[10px] border-collapse" style={{ minWidth: '100%' }}>
                  <thead>
                    <tr className="bg-navy/80 sticky top-0">
                      {ROSTER_COLUMNS.map(c => (
                        <th key={c.key} className="font-cond font-black tracking-widest text-muted px-2 py-1.5 text-left whitespace-nowrap border-b border-border">
                          {c.label}
                        </th>
                      ))}
                    </tr>
                  ...
                  <tbody>
                    {pending.rows.slice(0, 25).map((r, i) => (
                      <tr key={i} className={`border-b border-border/30 ${i % 2 === 0 ? '' : 'bg-white/5'}`}>
                        <td className="font-mono px-2 py-1 text-muted">{r.usa_lacrosse_number || '—'}</td>
                        ...
                        <td className="px-2 py-1 text-muted truncate max-w-[120px]">{r.email || '—'}</td>
                        <td className="px-2 py-1">
                          {r.div && <span className="font-cond font-bold text-blue-300">{r.div}</span>}
                        </td>
                        <td className="font-cond font-bold text-blue-300 px-2 py-1">{r.team || '—'}</td>
                        <td className="px-2 py-1 text-muted">{r.association || '—'}</td>
                        <td className="px-2 py-1">
                          <span className={`font-cond font-bold text-[9px] px-1.5 py-0.5 rounded ${r.type === 'Coach' ? 'bg-yellow-900/40 text-yellow-300' : 'bg-blue-900/30 text-blue-300'}`}>
                            {r.type || 'Player'}
                          </span>
                        </td>
                      </tr>
                    ...
                    {pending.rows.length > 25 && (
                      <tr><td colSpan={10} className="text-center text-muted text-[10px] py-2">
              ...
              {committed && (
                <div className="flex gap-2 p-3 border-t border-border">
                  <Btn variant="ghost" size="sm" onClick={() => { setPending(null); if (fileRef.current) fileRef.current.value = '' }}>
                    UPLOAD ANOTHER
                  </Btn>
                </div>
              )}
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            ...
            {teamId && teamPlayers.length > 0 && (
              <button onClick={() => {
                const lines = ['USA Lacrosse Number,Firstname,Lastname,Birthdate,Zipcode,Email,Div,Team,association,Player or Coach']
                const team = state.teams.find(t => t.id === teamId)
                teamPlayers.forEach(p => {
                  lines.push(`,,${p.name},,,,${team?.division ?? ''},${team?.name ?? ''},,${p.position === 'Coach' ? 'Coach' : 'Player'}`)
                })
          ...
          {!loadingTeam && teamId && (
            <div>
                ...
                <div className="flex justify-between items-center">
                  <div>
                    ...
                    <div className="font-cond text-[11px] text-muted">
                      {state.teams.find(t => t.id === teamId)?.division} · {state.teams.find(t => t.id === teamId)?.association ?? ''}
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="font-mono text-xl font-bold text-green-400">{teamPlayers.filter(p => p.position !== 'Coach').length}</div>
              ...
              <table className="w-full border-collapse">
                ...
                <tbody>
                  {/* Coaches first */}
                  {teamPlayers.filter(p => p.position === 'Coach').map(p => (
                    <tr key={p.id} className="border-b border-border/40 bg-yellow-900/10">
                      ...
                      <td className="font-cond font-bold text-[12px] text-yellow-300 px-3 py-1.5">{p.name}</td>
                      <td className="text-[11px] text-muted px-3 py-1.5">—</td>
                      <td className="px-3 py-1.5">
                        <span className="font-cond text-[9px] font-bold bg-yellow-900/40 text-yellow-300 px-1.5 py-0.5 rounded">COACH</span>
                      </td>
                    </tr>
                  ))}
                  {/* Players */}
                  {teamPlayers.filter(p => p.position !== 'Coach').map((p, i) => (
                    <tr key={p.id} className={`border-b border-border/40 ${i % 2 === 0 ? '' : 'bg-white/5'}`}>
                      ...
                      <td className="px-3 py-1.5">
                        <span className="font-cond text-[9px] font-bold bg-blue-900/30 text-blue-300 px-1.5 py-0.5 rounded">PLAYER</span>
                      </td>
                    </tr>
                  ))}
                </tbody>
