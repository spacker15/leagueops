...
import type { PublicGame, PublicTeam, PublicEventDate } from '@/lib/public-results/data'
    ...
    <div>
      ...
      <div className="flex items-center gap-1 mt-3 mb-4">
        {SUB_VIEWS.map((sv) => (
          <Link
            key={sv.id}
            href={`/results/e/${slug}?tab=schedule&view=${sv.id}&day=${activeDay}&div=${divFilter}`}
        ...
        <div className="flex items-center gap-1 overflow-x-auto pb-1 mb-4 no-scrollbar">
          {sortedDates.map((d) => {
            ...
            return (
              <Link
                key={d.id}
                href={`/results/e/${slug}?tab=schedule&view=${activeView}&day=${d.day_number}&div=${divFilter}`}
