...
import {
  RefreshCw, AlertTriangle, XCircle, CheckCircle, Shield,
  ChevronRight, Plus, Trash2, Clock, Calendar,
} from 'lucide-react'
...
export function ConflictsTab() {
  ...
  async function applyResolution(conflictId: number, action: string, params: Record<string, unknown>) {
    ...
    try {
      const res = await fetch('/api/field-engine', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          action: 'resolve', conflict_id: conflictId,
          resolution_action: action, resolution_params: params,
        }),
      })
      const data = await res.json()
      if (data.success) {
  ...
  async function addBlock() {
    ...
    if (error) { toast.error(error.message); setAddingBlock(false); return }
    ...
    setBlockField(''); setBlockStart(''); setBlockEnd(''); setBlockNotes('')
    await loadBlocks()
    ...
    <div>
        ...
        {SUBTABS.map(t => (
          <button key={t.id} onClick={() => setSubTab(t.id)}
            className={cn(
              ...
              t.id === 'conflicts' && conflicts.length > 0 && subTab !== t.id &&
      ...
      {engineResult && (
        <div className="grid grid-cols-5 gap-2 mb-4">
          {[
            { label: 'GAMES SCANNED',   value: engineResult.stats?.gamesScanned ?? 0,  color: 'text-blue-300' },
            { label: 'FIELDS SCANNED',  value: engineResult.stats?.fieldsScanned ?? 0, color: 'text-blue-300' },
            { label: 'OVERLAPS',        value: engineResult.stats?.overlapCount ?? 0,   color: engineResult.stats?.overlapCount > 0 ? 'text-red-400' : 'text-green-400' },
            { label: 'FIELD BLOCKS',    value: engineResult.stats?.blockedCount ?? 0,   color: engineResult.stats?.blockedCount > 0 ? 'text-yellow-400' : 'text-green-400' },
            { label: 'CASCADE RISK',    value: engineResult.stats?.cascadeCount ?? 0,   color: engineResult.stats?.cascadeCount > 0 ? 'text-orange-400' : 'text-green-400' },
          ].map(s => (
            <div key={s.label} className="bg-surface-card border border-border rounded-md p-2.5">
              <div className="font-cond text-[9px] font-bold tracking-widest text-muted uppercase mb-0.5">{s.label}</div>
        ...
        <div>
          <div className="flex justify-between items-center mb-3">
            <div className="font-cond text-[11px] font-bold tracking-widest text-muted uppercase">
              {conflicts.length === 0
                ...
                : <>
                    {criticalCount > 0 && <span className="text-red-400 mr-3">{criticalCount} CRITICAL</span>}
                    {warningCount  > 0 && <span className="text-yellow-400 mr-3">{warningCount} WARNINGS</span>}
          ...
          {conflicts.length === 0 ? (
            <div className="flex flex-col items-center py-16 gap-3">
              ...
              <div className="font-cond text-[12px] text-muted">Run the engine to scan for field conflicts</div>
            </div>
          ) : (
      ...
      {subTab === 'blocks' && (
        <div className="grid grid-cols-2 gap-6">
          ...
          <div>
            ...
            <div className="bg-surface-card border border-border rounded-lg p-4">
              <div className="grid grid-cols-2 gap-3 mb-3">
                <FormField label="Field">
                  <select
                    ...
                    value={blockField} onChange={e => setBlockField(e.target.value)}>
              ...
              <FormField label="Notes (optional)" className="mb-3">
                <input value={blockNotes} onChange={e => setBlockNotes(e.target.value)}
              ...
              <div className="text-[10px] text-muted font-cond mt-2 leading-relaxed">
                Field blocks prevent games from being scheduled during that window. Run the conflict engine after adding blocks to flag any existing games that conflict.
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            {blocks.length === 0 ? (
              <div className="text-center py-12 text-muted font-cond font-bold">NO FIELD BLOCKS SET</div>
            ) : (
              <div className="space-y-2">
                {blocks.map(block => (
                  <div key={block.id} className={cn(
                    ...
                    <div>
                      ...
                      <div className="font-cond text-[11px] text-muted">
                        ...
                        {new Date(block.starts_at).toLocaleString('en-US', { month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' })}
                        {' → '}
                        {new Date(block.ends_at).toLocaleString('en-US', { hour: 'numeric', minute: '2-digit' })}
                      </div>
                      {block.notes && (
      ...
      {subTab === 'history' && (
        <div>
          ...
          {runs.length === 0 ? (
            <div className="text-center py-12 text-muted font-cond">No engine runs yet — click RUN ENGINE to start</div>
          ) : (
                ...
                {runs.map(run => (
                  <tr key={run.id} className="border-b border-border/40 hover:bg-white/5">
                    <td className="font-mono text-blue-300 text-[11px] px-3 py-2">
                      {new Date(run.ran_at).toLocaleString('en-US', { month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' })}
                    </td>
                    <td className="font-cond font-bold px-3 py-2 uppercase">{run.engine_type}</td>
                    <td className="px-3 py-2">
                      {run.conflicts_found === 0
                        ? <span className="font-cond text-[11px] text-green-400 font-bold">0 — CLEAN</span>
                        : <span className={cn('font-mono font-bold', run.conflicts_found > 3 ? 'text-red-400' : 'text-yellow-400')}>{run.conflicts_found}</span>
                      }
                    </td>
                    <td className="font-mono text-muted text-[11px] px-3 py-2">{run.duration_ms}ms</td>
                    <td className="font-cond text-[11px] text-muted px-3 py-2">{run.triggered_by}</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
function ConflictCard({ conflict, onDismiss, onApply, applying }: {
          ...
          {conflict.impacted_game_ids?.length > 0 && (
            <div className="flex flex-wrap gap-1 mb-2">
              ...
              {conflict.impacted_game_ids.map(id => {
                return (
                  <span key={id} className="font-cond text-[10px] font-bold bg-white/10 text-muted px-2 py-0.5 rounded">
                    Game #{id}
                  </span>
                )
              })}
              {conflict.impacted_field_ids?.map(id => (
                <span key={id} className="font-cond text-[10px] font-bold bg-blue-900/30 text-blue-300 px-2 py-0.5 rounded">
                  Field {id}
                </span>
              ))}
            </div>
          ...
          {conflict.resolution_options?.length > 0 && (
            <div>
              <button onClick={() => setExpanded(e => !e)}
                ...
                {expanded ? 'HIDE RESOLUTION OPTIONS' : `${conflict.resolution_options.length} RESOLUTION OPTION${conflict.resolution_options.length > 1 ? 'S' : ''}`}
              </button>
              {expanded && (
