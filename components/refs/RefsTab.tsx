...
import {
  ...
  Clock, Shield, UserPlus, Trash2, Users, Calendar,
} from 'lucide-react'
...
function DraggablePerson({ id, name, role, type, checkedIn, small = false }: {
  id: string; name: string; role?: string; type: PersonType; checkedIn?: boolean; small?: boolean
}) {
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0">
        <div className={cn('font-cond font-bold leading-none truncate', small ? 'text-[10px]' : 'text-[11px]')}>
          {name.split(' ')[0]} {name.split(' ')[1]?.[0]}.
        </div>
        {role && !small && <div className="font-cond text-[9px] opacity-60 leading-none mt-0.5">{role}</div>}
      </div>
...
// ─── Droppable game card ─────────────────────────────────────
function DroppableGameCard({ game, assignments, onRemove, isOver }: {
  game: Game
    ...
    >
      ...
      <div className={cn(
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="font-mono text-[10px] text-blue-300 font-bold">{game.scheduled_time}</span>
        </div>
        <div className="flex items-center gap-1">
          ...
          <span className={cn('font-cond text-[9px] font-black tracking-wider px-1.5 py-0.5 rounded',
            ...
            game.status === 'Halftime'? 'badge-halftime' : 'badge-scheduled'
      ...
      <div className="px-2.5 py-1.5">
        <div className="font-cond font-black text-[12px] text-white leading-tight mb-1.5">
          {game.home_team?.name ?? '?'} <span className="text-muted font-normal text-[10px]">vs</span> {game.away_team?.name ?? '?'}
        </div>
...
function AssignedChip({ assignment, onRemove }: { assignment: Assignment; onRemove: (a: Assignment) => void }) {
  return (
    <div className={cn(
      ...
      <button onClick={() => onRemove(assignment)} className="hover:text-white ml-0.5 opacity-60 hover:opacity-100">
        <XCircle size={10} />
      </button>
    </div>
  )
...
function DroppableFieldHeader({ fieldId, fieldName, isOver }: {
  fieldId: number; fieldName: string; isOver: boolean
}) {
...
export function RefsTab() {
  ...
  const [roleModal, setRoleModal]       = useState<{
    personId: number; personType: PersonType; personName: string;
    targetType: 'game' | 'field'; targetId: number; targetName: string
  } | null>(null)
  ...
  function handleDragEnd(e: DragEndEvent) {
    ...
    if (overId.startsWith('game-')) {
      ...
      const already = assignments.find(a =>
        a.game_id === gameId &&
        (a.referee_id === personId || a.volunteer_id === personId)
      )
      if (already) { toast('Already assigned to this game'); return }

      const defaultRole = personType === 'ref'
        ? (assignments.filter(a => a.game_id === gameId && a.person_type === 'ref').length === 0 ? 'Center' : 'Trail')
        : (state.volunteers.find(v => v.id === personId)?.role ?? 'Score Table')

      setRoleModal({
        personId, personType, personName: person.name,
      ...
      setSelectedRole(personType === 'ref' ? 'Center' : (state.volunteers.find(v => v.id === personId)?.role ?? 'Score Table'))
  ...
  async function confirmAssignment() {
    ...
    if (targetType === 'game') {
      ...
      if (personType === 'ref') {
        const { data, error } = await sb.from('ref_assignments')
          .upsert({ game_id: targetId, referee_id: personId, role: selectedRole }, { onConflict: 'game_id,referee_id' })
          .select().single()
        if (error) { toast.error(error.message); return }
        setAssignments(prev => [...prev.filter(a => !(a.game_id === targetId && a.referee_id === personId)),
          { id: (data as any).id, game_id: targetId, referee_id: personId, role: selectedRole, person_name: personName, person_type: 'ref' }])
      } else {
        const { data, error } = await sb.from('vol_assignments')
          .upsert({ game_id: targetId, volunteer_id: personId }, { onConflict: 'game_id,volunteer_id' })
          .select().single()
        if (error) { toast.error(error.message); return }
        setAssignments(prev => [...prev.filter(a => !(a.game_id === targetId && a.volunteer_id === personId)),
          { id: (data as any).id, game_id: targetId, volunteer_id: personId, role: selectedRole, person_name: personName, person_type: 'vol' }])
      }
      ...
      for (const game of fieldGames) {
        ...
        if (personType === 'ref') {
          const { data, error } = await sb.from('ref_assignments')
            .upsert({ game_id: game.id, referee_id: personId, role: selectedRole }, { onConflict: 'game_id,referee_id' })
            .select().single()
          if (!error && data) {
            newAssigns.push({ id: (data as any).id, game_id: game.id, referee_id: personId, role: selectedRole, person_name: personName, person_type: 'ref' })
            count++
          }
        } else {
          const { data, error } = await sb.from('vol_assignments')
            .upsert({ game_id: game.id, volunteer_id: personId }, { onConflict: 'game_id,volunteer_id' })
            .select().single()
          if (!error && data) {
            newAssigns.push({ id: (data as any).id, game_id: game.id, volunteer_id: personId, role: selectedRole, person_name: personName, person_type: 'vol' })
            count++
          }
        }
      }
      ...
      setBlockAssignments(prev => [...prev, {
        id: blockId, person_id: personId, person_type: personType,
        person_name: personName, field_id: targetId,
        ...
        role: selectedRole, game_count: count,
      }])
  ...
  async function removeBlockAssignment(block: BlockAssignment) {
    ...
    if (block.person_type === 'ref') {
      await sb.from('ref_assignments').delete().in('game_id', gameIds).eq('referee_id', block.person_id)
    } else {
      await sb.from('vol_assignments').delete().in('game_id', gameIds).eq('volunteer_id', block.person_id)
    }
    setAssignments(prev => prev.filter(a =>
      !(gameIds.includes(a.game_id) &&
        (block.person_type === 'ref' ? a.referee_id === block.person_id : a.volunteer_id === block.person_id))
    ))
  ...
  async function runEngine() {
    ...
    try {
      ...
      else { toast(`${data.conflicts.length} conflicts`, { icon: '⚠️' }); setSubTab('conflicts') }
    } catch (err: any) {
  ...
  async function resolveConflict(id: number) {
    ...
    if (res.ok) { setConflicts(prev => prev.filter(c => c.id !== id)); toast.success('Resolved') }
  }

  async function openAvailability(ref: Referee) {
    setSelectedRef(ref)
    const sb = createClient()
    const { data } = await sb.from('referee_availability').select('*').eq('referee_id', ref.id).order('date')
  ...
  async function saveAvailability() {
    ...
    const sb = createClient()
    await sb.from('referee_availability').upsert(
    ...
    setAvailability((data as RefereeAvailability[]) ?? [])
    toast.success('Saved'); setNewDate('')
  }

  // Group games by field
  const fieldColumns = state.fields.map(field => ({
    field,
    games: state.games
  ...
  return (
    <div>
        ...
        {SUBTABS.map(t => (
          <button key={t.id} onClick={() => setSubTab(t.id)}
            className={cn(
              ...
              subTab === t.id ? 'border-red text-white' : 'border-transparent text-muted hover:text-white',
          ...
          <div className="flex gap-3 h-full" style={{ minHeight: 0 }}>
            ...
            <div className="w-52 flex-shrink-0">
              ...
              <div className="bg-navy/40 border border-border rounded-md p-2.5 mb-3 text-[10px] text-muted font-cond leading-relaxed">
                ...
                <div>• Drag to a <span className="text-blue-300">game card</span> → single game</div>
                <div>• Drag to a <span className="text-blue-300">field header</span> → all games on that field</div>
              ...
              {blockAssignments.length > 0 && (
                <div className="mb-3">
                  <div className="font-cond text-[10px] font-black tracking-widest text-muted uppercase mb-1.5">BLOCK ASSIGNMENTS</div>
                  {blockAssignments.map(b => (
                    <div key={b.id} className={cn(
                      ...
                      <div>
                        <div className="font-cond font-bold text-[11px] text-white">{b.person_name.split(' ')[0]}</div>
                        <div className="text-muted">{b.field_name} · {b.game_count} games</div>
                        <div className="text-muted">{b.role}</div>
                      </div>
                      <button onClick={() => removeBlockAssignment(b)} className="text-muted hover:text-red-400 mt-0.5">
                        <XCircle size={12} />
                      </button>
                    </div>
                  ))}
              ...
              <div className="font-cond text-[10px] font-black tracking-widest text-muted uppercase mb-1.5">
                REFEREES ({state.referees.filter(r => r.checked_in).length}/{state.referees.length} IN)
              </div>
              ...
              {/* Volunteers pool */}
              <div className="font-cond text-[10px] font-black tracking-widest text-muted uppercase mb-1.5">
                VOLUNTEERS ({state.volunteers.filter(v => v.checked_in).length}/{state.volunteers.length} IN)
              </div>
            ...
            {draggingPerson && (
              <div className={cn(
                ...
                <div className="w-4 h-4 rounded-full bg-white/20 flex items-center justify-center text-[8px] font-black">
                  {draggingPerson.name.split(' ').map((n: string) => n[0]).join('').slice(0, 2)}
                </div>
          ...
          {state.referees.map(ref => {
            ...
            return (
              <div key={ref.id} className={cn(
                ...
                ref.checked_in ? 'bg-green-900/10 border-green-800/40' : 'bg-surface-card border-border'
              )}>
                ...
                {rd.eligible_divisions?.length > 0 && (
                  <div className="flex flex-wrap gap-1 mb-2">
                    {(rd.eligible_divisions as string[]).map((d: string) => (
                      <span key={d} className="font-cond text-[9px] font-bold bg-blue-900/30 text-blue-300 px-1.5 py-0.5 rounded">{d}</span>
                    ))}
                  </div>
                )}
                {rd.certifications?.length > 0 && (
                  <div className="font-cond text-[9px] text-muted mb-2">{(rd.certifications as string[]).join(' · ')}</div>
                )}
                <div className="flex gap-1 mt-2">
                  <button onClick={() => toggleRefCheckin(ref.id)}
                  ...
                  <button onClick={() => openAvailability(ref)} title="Availability"
      ...
      {subTab === 'volunteers' && (
        <div>
          <div className="grid grid-cols-[repeat(auto-fill,minmax(210px,1fr))] gap-2 mb-6">
            {state.volunteers.map(vol => (
              <div key={vol.id} onClick={() => toggleVolCheckin(vol.id)}
                className={cn(
                  'flex gap-2 items-start p-3 rounded-md border cursor-pointer transition-all',
                  vol.checked_in ? 'bg-green-900/10 border-green-800/40' : 'bg-surface-card border-border hover:border-blue-400'
                )}>
          ...
          <div className="font-cond text-[10px] font-black tracking-widest text-muted uppercase mb-2">COVERAGE BY ROLE</div>
            ...
            {['Score Table','Clock','Field Marshal','Operations','Gate'].map(role => {
              ...
              return (
                <div key={role} className="bg-surface-card border border-border rounded-md p-3">
                  ...
                  <div className={cn('font-mono text-xl font-bold', pct===100?'text-green-400':pct>=50?'text-yellow-400':'text-red-400')}>{checked}/{total}</div>
                  <div className="h-1 bg-white/10 rounded mt-2 overflow-hidden">
                    <div className="h-full rounded" style={{ width:`${pct}%`, background:pct===100?'#22c55e':pct>=50?'#facc15':'#f87171' }} />
                  </div>
                </div>
              )
            })}
            ...
            <div className="font-cond text-[11px] font-bold tracking-widest text-muted uppercase">
              {conflicts.length === 0
                ...
                : <>{criticalCount > 0 && <span className="text-red-400 mr-2">{criticalCount} CRITICAL</span>}
                   {conflicts.filter(c=>c.severity==='warning').length > 0 && <span className="text-yellow-400">{conflicts.filter(c=>c.severity==='warning').length} WARNINGS</span>}</>}
            </div>
            <Btn size="sm" variant="ghost" onClick={loadConflicts}>REFRESH</Btn>
          </div>
          {conflicts.length === 0
            ? <div className="flex flex-col items-center py-16 gap-3"><CheckCircle size={48} className="text-green-400" /><div className="font-cond font-black text-[18px] text-green-400">ALL CLEAR</div></div>
            : <div className="space-y-2">{conflicts.map(c => <ConflictCard key={c.id} conflict={c} onResolve={() => resolveConflict(c.id)} />)}</div>
          }
        </div>
      )}
      ...
      {subTab === 'availability' && (
        <div>
          <div className="font-cond text-[11px] text-muted mb-4">
            Set availability windows per referee. The engine uses these to detect assignment conflicts.
          </div>
          <div className="grid grid-cols-[repeat(auto-fill,minmax(240px,1fr))] gap-3">
            {state.referees.map(ref => <RefAvailCard key={ref.id} ref_={ref} onEdit={() => openAvailability(ref)} />)}
          </div>
        </div>
      )}
      ...
      <Modal
        ...
        footer={
          <>
            <Btn variant="ghost" size="sm" onClick={() => setRoleModal(null)}>CANCEL</Btn>
            <Btn variant="primary" size="sm" onClick={confirmAssignment}>CONFIRM ASSIGNMENT</Btn>
          </>
        }
      >
        {roleModal && (
            ...
            <div className="bg-surface-elevated rounded-md p-3 mb-4">
              <div className="flex items-center gap-3">
                <div className={cn(
                  ...
                  roleModal.personType === 'ref' ? 'bg-red-900/40 text-red-300' : 'bg-blue-900/40 text-blue-300'
                )}>
                  {roleModal.personName.split(' ').map(n => n[0]).join('').slice(0,2)}
                </div>
                <div>
          ...
          <div>
            ...
            {availability.length === 0
              ...
              : <table className="w-full border-collapse text-[12px]">
                  <thead><tr className="bg-navy">{['DATE','FROM','TO',''].map(h=><th key={h} className="font-cond text-[10px] font-black tracking-widest text-muted px-3 py-1.5 text-left">{h}</th>)}</tr></thead>
                  <tbody>
                    {availability.map(a => (
                      <tr key={a.id} className="border-b border-border/40">
                        ...
                        <td className="px-3 py-2">
                          ...
                          }} className="text-muted hover:text-red-400"><XCircle size={14} /></button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
...
function ConflictCard({ conflict, onResolve }: { conflict: OperationalConflict; onResolve: () => void }) {
  ...
  const LABELS: Record<string,string> = {
    ref_double_booked:'DOUBLE BOOKED', ref_unavailable:'UNAVAILABLE', max_games_exceeded:'MAX GAMES',
    missing_referee:'MISSING REF', field_overlap:'FIELD OVERLAP', weather_closure:'WEATHER',
  }
  return (
      ...
      <div className="flex items-start gap-2">
        ...
        <div className="flex-1">
          <div className="flex justify-between mb-1">
            <span className={cn('font-cond font-black text-[11px] tracking-widest', s.text)}>{LABELS[conflict.conflict_type] ?? conflict.conflict_type.toUpperCase()}</span>
            <span className="font-mono text-[9px] text-muted">{new Date(conflict.created_at).toLocaleTimeString('en-US',{hour:'numeric',minute:'2-digit'})}</span>
          </div>
          <div className="text-[12px] text-gray-200 mb-2">{conflict.description}</div>
          {conflict.impacted_game_ids.length > 0 && (
            <div className="flex flex-wrap gap-1 mb-2">
              {conflict.impacted_game_ids.map(id=><span key={id} className="font-cond text-[10px] font-bold bg-white/10 text-muted px-1.5 py-0.5 rounded">Game #{id}</span>)}
            </div>
          )}
          {conflict.resolution_options?.length > 0 && (
            <div>
              <button onClick={()=>setExpanded(e=>!e)} className="font-cond text-[10px] font-bold text-blue-300 mb-1">{expanded?'▲ HIDE':'▼ RESOLUTIONS'}</button>
              {expanded && <div className="flex flex-wrap gap-1.5 mt-1">{conflict.resolution_options.map((o,i)=><button key={i} className="font-cond text-[10px] font-bold px-2 py-1 rounded bg-navy hover:bg-navy-light text-white">{o.label}</button>)}</div>}
            </div>
          )}
        </div>
        <button onClick={onResolve} className="shrink-0 font-cond text-[10px] font-bold px-2 py-1 rounded bg-green-900/40 text-green-400 border border-green-800/50 hover:bg-green-800/60 ml-2">RESOLVE</button>
      </div>
    </div>
  )
}
  ...
  return (
    <div className="bg-surface-card border border-border rounded-md p-3">
      <div className="flex justify-between items-center mb-2">
        <div><div className="font-cond font-black text-[13px]">{ref_.name}</div><div className="font-cond text-[10px] text-muted">{ref_.grade_level}</div></div>
        <Btn size="sm" variant="ghost" onClick={onEdit}>EDIT</Btn>
      </div>
      {windows.length === 0
        ? <div className="text-[10px] text-muted font-cond italic">No windows set</div>
        : <div className="space-y-1">{windows.map(w=><div key={w.id} className="flex justify-between text-[10px]"><span className="font-mono text-blue-300">{w.date}</span><span className="text-muted">{w.available_from}–{w.available_to}</span></div>)}</div>
      }
    </div>
  )
}
