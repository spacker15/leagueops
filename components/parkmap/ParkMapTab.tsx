...
import { Upload, X, MapPin, Map, Image as ImageIcon, ExternalLink, Save, Trash2, RotateCw, Plus } from 'lucide-react'
...
export function ParkMapTab() {
  ...
  async function saveSettings() {
    ...
    if (photoFile) {
      ...
      if (error) { toast.error(`Upload failed: ${error.message}`); setSaving(false); setUploading(false); return }
    ...
    else { toast.success('Map settings saved'); setShowSettings(false) }
    setSaving(false)
  }

  // ── Pointer event handlers ──────────────────────────────────
  function getCanvasRect() {
    return canvasRef.current?.getBoundingClientRect() ?? { left: 0, top: 0, width: 800, height: 540 }
  }
  ...
  const embedSrc = embedCode
    ? (embedCode.includes('src="')
        ...
        : embedCode.startsWith('http') ? embedCode : null)
    : null
    ...
    <div>
      ...
      <div className="flex items-center gap-2 mb-3 flex-wrap">
        ...
        <div className="flex rounded overflow-hidden border border-border">
          ...
          ] as { id: MapView; label: string; disabled?: boolean }[]).map(v => (
      ...
      {showSettings && (
        <div className="bg-surface-card border border-border rounded-xl p-5 mb-4">
          ...
          <div className="grid grid-cols-2 gap-5">
            {/* Photo */}
            <div>
              <div className="font-cond text-[10px] font-bold tracking-widest text-muted uppercase mb-2">PARK PHOTO</div>
              {photoPreview ? (
                <div className="relative rounded-lg overflow-hidden border border-border mb-2" style={{ height: 150 }}>
                  <img src={photoPreview} alt="Park" className="w-full h-full object-cover" />
                  <button onClick={() => { setPhotoPreview(null); setPhotoFile(null); setSavedPhotoUrl(null) }}
              ...
              {photoFile && <div className="font-cond text-[10px] text-blue-300 mt-1">✓ {photoFile.name}</div>}
            ...
            {/* Google Maps */}
            <div>
              <div className="font-cond text-[10px] font-bold tracking-widest text-muted uppercase mb-2">GOOGLE MAPS</div>
              <div className="space-y-3">
                <div>
                  ...
                  <input className="w-full bg-surface border border-border text-white px-2.5 py-2 rounded text-[12px] outline-none focus:border-blue-400"
                    value={parkName} onChange={e => setParkName(e.target.value)} placeholder="Riverside Sports Complex" />
                </div>
                <div>
                  <label className="font-cond text-[10px] text-muted block mb-1">Google Maps URL (share link)</label>
                  <input className="w-full bg-surface border border-border text-white px-2.5 py-2 rounded text-[12px] outline-none focus:border-blue-400"
                    value={mapsUrl} onChange={e => setMapsUrl(e.target.value)} placeholder="https://maps.google.com/..." />
                </div>
                <div>
                  <label className="font-cond text-[10px] text-muted block mb-1 flex justify-between">
                    <span>Embed Code</span>
                    <a href="https://www.google.com/maps" target="_blank" rel="noopener noreferrer" className="text-blue-300 text-[9px]">Get from Google Maps →</a>
                  </label>
                  <textarea className="w-full bg-surface border border-border text-white px-2.5 py-2 rounded text-[11px] outline-none focus:border-blue-400 resize-none h-20 font-mono"
          ...
          <div className="flex justify-end gap-3 mt-4 pt-4 border-t border-border">
            <button onClick={() => setShowSettings(false)} className="font-cond text-[12px] text-muted hover:text-white px-4 py-2">CANCEL</button>
      ...
      >
        ...
        {view === 'overlay' && (
          <>
            <div className="absolute inset-0" style={{ background: 'linear-gradient(150deg,#071a07 0%,#0a2a0a 100%)' }} />
            <svg className="absolute inset-0 w-full h-full pointer-events-none" style={{ opacity: 0.06 }}>
              {Array.from({ length: 26 }, (_, i) => <line key={`v${i}`} x1={i*40} y1={0} x2={i*40} y2={540} stroke="#7bb8ff" strokeWidth={0.5} />)}
              {Array.from({ length: 14 }, (_, i) => <line key={`h${i}`} x1={0} y1={i*40} x2={1040} y2={i*40} stroke="#7bb8ff" strokeWidth={0.5} />)}
            </svg>
          </>
        )}
        ...
        {view === 'photo' && !photoPreview && (
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-navy/20">
            ...
            <button onClick={() => setShowSettings(true)} className="font-cond text-[12px] font-bold text-blue-300 hover:text-white">
        ...
        {view === 'satellite' && !embedSrc && (
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-navy/20">
            ...
            {mapsUrl
              ? <a href={mapsUrl} target="_blank" rel="noopener noreferrer" className="font-cond text-[12px] font-bold text-blue-300 hover:text-white flex items-center gap-1.5"><ExternalLink size={12} /> Open in Google Maps</a>
              : <button onClick={() => setShowSettings(true)} className="font-cond text-[12px] font-bold text-blue-300 hover:text-white">Add Google Maps embed code in Settings →</button>
            }
          </div>
        )}
        ...
        {state.fields.map(field => {
          ...
          return (
            <div key={field.id}
              ...
              onClick={e => { e.stopPropagation(); setSelected(isSel ? null : field.id) }}
              onDoubleClick={e => { e.stopPropagation(); handleRename(field.id) }}
            >
              {/* Field body */}
              <div
              ...
              >
                <div className="font-cond text-3xl font-black text-white/25 leading-none pointer-events-none">{field.number}</div>
                <div className="font-cond font-black text-[13px] text-white text-center px-2 leading-tight pointer-events-none">{field.name}</div>
                {active && <div className="font-cond text-[9px] text-green-200 text-center px-1 truncate w-full pointer-events-none">{active}</div>}
              </div>
              ...
              {isSel && (
                <>
                  ...
                  {([
                    { handle: 'resize-nw' as Handle, style: { top: -5, left: -5, cursor: 'nw-resize' } },
                    { handle: 'resize-ne' as Handle, style: { top: -5, right: -5, cursor: 'ne-resize' } },
                    { handle: 'resize-sw' as Handle, style: { bottom: -5, left: -5, cursor: 'sw-resize' } },
                    { handle: 'resize-se' as Handle, style: { bottom: -5, right: -5, cursor: 'se-resize' } },
                  ]).map(({ handle, style }) => (
                    <div key={handle}
                      style={{ position: 'absolute', width: 12, height: 12, background: '#60a5fa', border: '2px solid white', borderRadius: 2, zIndex: 30, ...style }}
                      onPointerDown={e => { e.stopPropagation(); onPointerDown(e, field.id, handle) }}
                    />
                  ))}

                  {/* Rotate handle */}
                  <div style={{ position: 'absolute', top: -28, left: '50%', transform: 'translateX(-50%)', cursor: 'grab', zIndex: 30, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}
                    onPointerDown={e => { e.stopPropagation(); onPointerDown(e, field.id, 'rotate') }}>
                    <div style={{ width: 1, height: 10, background: '#60a5fa' }} />
                    <div style={{ width: 18, height: 18, background: '#60a5fa', borderRadius: '50%', border: '2px solid white', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      <RotateCw size={10} color="white" />
                    </div>
                  </div>

                  {/* Selection border glow */}
                  <div className="absolute inset-0 rounded-md pointer-events-none" style={{ boxShadow: '0 0 0 2px #60a5fa, 0 0 12px rgba(96,165,250,0.4)' }} />
                </>
              )}
            </div>
          )
        ...
        <div className="absolute bottom-3 right-3 bg-black/50 backdrop-blur-sm rounded-lg px-3 py-1.5 pointer-events-none">
          <div className="font-cond text-[9px] text-muted font-bold">DRAG · CORNER = RESIZE · ↻ = ROTATE</div>
        </div>
      </div>
        ...
        <div className="mt-3 bg-surface-card border border-border rounded-xl p-3">
          <div className="flex items-center gap-4 flex-wrap">
            ...
            <div className="flex items-center gap-1.5">
              <span className="font-cond text-[9px] text-muted uppercase tracking-wider mr-1">COLOR</span>
              {FIELD_COLORS.map(fc => (
                <button key={fc.value} onClick={() => updateFieldFull(sel.id, { map_color: fc.value })}
                  title={fc.label}
              ...
              {/* Custom color picker */}
              <label className="relative w-6 h-6 rounded-full border-2 border-border hover:border-white cursor-pointer overflow-hidden" title="Custom color">
                <div className="w-full h-full rounded-full" style={{ background: 'conic-gradient(red,yellow,lime,cyan,blue,magenta,red)' }} />
                <input type="color" value={sel.map_color ?? '#1a6b1a'}
            ...
            <div className="flex items-center gap-2">
              <span className="font-cond text-[9px] text-muted uppercase tracking-wider">OPACITY</span>
              <input type="range" min={20} max={100} value={sel.map_opacity ?? 70}
            ...
            <div className="flex items-center gap-2">
              <span className="font-cond text-[9px] text-muted uppercase tracking-wider">ROTATE</span>
              <input type="number" min={0} max={359} value={sel.map_rotation ?? 0}
            ...
            <div className="flex gap-2 ml-auto">
              <button onClick={() => handleRename(sel.id)}
                className="font-cond text-[11px] font-bold px-3 py-1.5 rounded border border-border text-muted hover:text-white transition-colors">
                RENAME
              </button>
              <button onClick={() => deleteField(sel.id)}
