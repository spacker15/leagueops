...
import {
  conditionIcon, windDirection, evaluateAlerts, calcHeatIndex,
  THRESHOLDS, type WeatherReading, type WeatherAlert,
} from '@/lib/engines/weather'
import {
  Zap, Thermometer, Wind, Droplets, Eye, RefreshCw,
  AlertTriangle, CheckCircle, XCircle, Shield, Activity,
...
export function WeatherTab() {
  ...
  useEffect(() => {
    const sb = createClient()
    const sub = sb.channel('weather-tab')
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'weather_alerts' }, () => loadAlerts())
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'weather_alerts' }, () => loadAlerts())
      .subscribe()
    ...
    try {
      ...
      if (data.lightning_active) {
        toast(`⚡ LIGHTNING DETECTED — ${data.games_affected} games suspended`, {
          style: { background: '#6b0000', color: 'white' }, duration: 6000
        })
      } else if (data.alerts.length > 0) {
        toast.error(`${data.alerts.length} weather alert${data.alerts.length > 1 ? 's' : ''} issued`)
      } else {
        toast.success(`${data.reading.complex_name} — ${data.reading.conditions}, ${data.reading.temperature_f}°F`)
      }
    } catch (err: any) {
  ...
  async function manualLightning(complexId: number, action: 'trigger' | 'lift') {
    ...
    if (res.ok) {
      if (action === 'trigger') {
        toast('⚡ LIGHTNING DELAY INITIATED', { style: { background: '#6b0000', color: 'white' }, duration: 5000 })
        setLightningStatus(prev => ({ ...prev, [complexId]: { active: true, event: null, secondsLeft: 1800 } }))
      } else {
        toast.success('Lightning delay lifted — fields resuming')
        setLightningStatus(prev => ({ ...prev, [complexId]: { active: false, event: null, secondsLeft: 0 } }))
      }
      await loadAlerts()
    ...
    <div>
      ...
      <div className="flex items-center gap-0 mb-4 border-b border-border">
        ...
        <div className="ml-auto flex items-center gap-2 pb-2">
          <Btn variant="ghost" size="sm" onClick={() => setShowApiInput(s => !s)}>API KEY</Btn>
          <Btn variant="primary" size="sm" onClick={scanAll} disabled={scanning !== null}>
            <RefreshCw size={11} className={cn('inline mr-1', scanning !== null && 'animate-spin')} />
            SCAN ALL
          </Btn>
        </div>
      </div>
      ...
      {showApiInput && (
        <div className="bg-surface-card border border-border rounded-md p-3 mb-4 flex gap-3 items-end">
          ...
          <div className="text-[10px] text-muted font-cond pb-1.5">
            Get free key at<br />openweathermap.org
          </div>
        </div>
      )}
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {delayedGames.map(game => (
                    <div key={game.id} className="bg-red-900/15 border border-l-4 border-red-500 border-red-900/40 rounded-md p-3">
          ...
          <div>
            ...
            {activeAlerts.length === 0 ? (
              <div className="flex flex-col items-center py-8 gap-2">
                ...
                <div className="font-cond font-bold text-[12px] text-green-400 tracking-wide">ALL CLEAR</div>
              </div>
            ) : (
            ...
            <div className="mt-4">
              ...
              <div className="space-y-1">
                {[
                  { range: '< 95°F',    label: 'Normal play',          color: 'text-green-400',  bg: 'bg-green-900/15' },
                  { range: '95–103°F',  label: 'Advisory — water breaks', color: 'text-yellow-400', bg: 'bg-yellow-900/15' },
                  { range: '103–113°F', label: 'Warning — breaks req.', color: 'text-orange-400', bg: 'bg-orange-900/15' },
                  { range: '> 113°F',   label: 'Emergency — suspend',   color: 'text-red-400',    bg: 'bg-red-900/15' },
                ].map(item => (
                  <div key={item.range} className={cn('flex justify-between px-2.5 py-1.5 rounded text-[11px]', item.bg)}>
            ...
            <div className="mt-4">
              ...
              <div className="space-y-1">
                {[
                  { range: '< 25 mph',  label: 'Normal play',       color: 'text-green-400',  bg: 'bg-green-900/15' },
                  { range: '25–40 mph', label: 'Advisory — monitor', color: 'text-yellow-400', bg: 'bg-yellow-900/15' },
                  { range: '> 40 mph',  label: 'Suspend play',       color: 'text-red-400',    bg: 'bg-red-900/15' },
                ].map(item => (
                  <div key={item.range} className={cn('flex justify-between px-2.5 py-1.5 rounded text-[11px]', item.bg)}>
                    <span className={cn('font-mono font-bold', item.color)}>{item.range}</span>
                    <span className="text-muted font-cond font-bold">{item.label}</span>
            ...
            return (
              <div key={complex.id} className="bg-surface-card border border-border rounded-lg p-4">
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <div className="font-cond font-black text-[16px] text-white">{complex.name}</div>
                    <div className="font-cond text-[11px] text-muted mt-0.5">{complex.address ?? 'No address set'}</div>
                    <div className="font-cond text-[10px] text-muted mt-0.5">
                      GPS: {complex.lat ?? '—'}, {complex.lng ?? '—'} · Lightning radius: {complex.lightning_radius_miles} mi
                    </div>
                  </div>
                  <div className="flex gap-2">
                    <Btn size="sm" variant="ghost" onClick={() => { loadHistory(complex.id); setSubTab('history') }}>
                      HISTORY
                    </Btn>
                    <Btn size="sm" variant="primary" onClick={() => runWeatherScan(complex.id)} disabled={scanning === complex.id}>
                      <RefreshCw size={10} className={cn('inline mr-1', scanning === complex.id && 'animate-spin')} />
                      SCAN
                    </Btn>
                  </div>
                </div>
                ...
                <div className="mb-4">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {complexFields.map(f => {
                      ...
                      const liveGame   = fieldGames.find(g => g.status === 'Live' || g.status === 'Halftime')
                      const delayed    = fieldGames.some(g => g.status === 'Delayed')
                      return (
                        <div key={f.id} className={cn(
                          'font-cond text-[11px] font-bold px-2.5 py-1 rounded border',
                          delayed   ? 'bg-red-900/20 border-red-700/50 text-red-300' :
                          liveGame  ? 'bg-green-900/20 border-green-700/50 text-green-300' :
                    ...
                    {complexFields.length === 0 && (
                      <span className="text-[10px] text-muted font-cond italic">No fields assigned to this complex</span>
                    )}
                  </div>
                </div>
      ...
      {subTab === 'history' && (
        <div>
          ...
          {history.length === 0 ? (
            <div className="text-center py-12 text-muted font-cond">Select a complex to load history</div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full border-collapse text-[11px]">
                <thead>
                  <tr className="bg-navy">
                    {['TIME','TEMP','FEELS','HEAT IDX','HUMIDITY','WIND','GUSTS','CONDITIONS','LIGHTNING'].map(h => (
                      <th key={h} className="font-cond text-[10px] font-black tracking-widest text-muted px-3 py-2 text-left border-b-2 border-border whitespace-nowrap">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {history.slice().reverse().map((r: any) => (
                    <tr key={r.id} className={cn(
                      ...
                      <td className="font-mono text-blue-300 px-3 py-2 whitespace-nowrap">
                        {new Date(r.fetched_at).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' })}
                      </td>
                      <td className="font-mono px-3 py-2">{r.temperature_f}°F</td>
                      <td className="font-mono text-muted px-3 py-2">{r.feels_like_f}°F</td>
                      <td className={cn('font-mono font-bold px-3 py-2',
                        r.heat_index_f >= 113 ? 'text-red-400' :
                        r.heat_index_f >= 103 ? 'text-orange-400' :
                        r.heat_index_f >= 95  ? 'text-yellow-400' : 'text-green-400'
                      )}>{r.heat_index_f}°F</td>
                      ...
                      <td className="font-cond px-3 py-2 text-muted capitalize">{r.conditions}</td>
                      <td className="px-3 py-2">
                        {r.lightning_detected
                          ? <span className="font-cond text-[10px] font-bold text-red-400">⚡ YES</span>
...
function ComplexWeatherCard({ complex, reading, lightningStatus: ls, scanning, onScan, onLightning }: {
  complex: Complex
    ...
    <div className={cn(
        ...
        <div className="flex items-center gap-2">
          {complex.last_weather_fetch && (
            <span className="font-cond text-[9px] text-muted">
              Updated {new Date(complex.last_weather_fetch).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' })}
            </span>
          )}
          <Btn size="sm" variant={ls?.active ? 'danger' : 'ghost'}
      ...
      {ls?.active && (
        <div className="bg-red-900/20 px-4 py-2 flex items-center justify-between border-b border-red-900/40">
          ...
          <div className="font-mono text-2xl font-bold text-red-400">
            {Math.floor((ls.secondsLeft ?? 0) / 60)}:{((ls.secondsLeft ?? 0) % 60).toString().padStart(2, '0')}
          </div>
      ...
      {reading ? (
        ...
        <div className="p-4 text-[11px] text-muted font-cond text-center">
          Click SCAN to fetch weather data{!complex.lat ? ' (add GPS coordinates for live data)' : ''}
        </div>
      )}
    </div>
  )
...
function WeatherReadingGrid({ reading, compact = false }: { reading: WeatherReading; compact?: boolean }) {
  const heatColor = reading.heat_index_f >= 113 ? 'text-red-400' :
                    reading.heat_index_f >= 103 ? 'text-orange-400' :
                    reading.heat_index_f >= 95  ? 'text-yellow-400' : 'text-green-400'

  const windColor = reading.wind_mph >= 40  ? 'text-red-400' :
                    reading.wind_mph >= 25  ? 'text-yellow-400' : 'text-green-400'

  const stats = [
    { icon: <Thermometer size={14} />, label: 'TEMP',        value: `${reading.temperature_f}°F`, color: 'text-white' },
    { icon: <Thermometer size={14} />, label: 'HEAT IDX',    value: `${reading.heat_index_f}°F`,  color: heatColor },
    { icon: <Droplets size={14} />,    label: 'HUMIDITY',    value: `${reading.humidity_pct}%`,    color: 'text-blue-300' },
    { icon: <Wind size={14} />,        label: 'WIND',        value: `${reading.wind_mph} mph ${windDirection(reading.wind_dir_deg)}`, color: windColor },
    { icon: <Wind size={14} />,        label: 'GUSTS',       value: `${reading.wind_gust_mph} mph`, color: windColor },
    { icon: <Eye size={14} />,         label: 'VISIBILITY',  value: `${reading.visibility_mi} mi`, color: 'text-muted' },
  ]
    ...
    <div>
      <div className="flex items-center gap-2 mb-3">
        ...
        <div>
          ...
          <div className="font-cond text-[10px] text-muted">
            {reading.source === 'live' ? '🟢 LIVE DATA' : reading.source === 'cache' ? '🟡 CACHED' : '🔵 SIMULATED'}
      ...
      <div className={cn('grid gap-2', compact ? 'grid-cols-6' : 'grid-cols-3')}>
        {stats.map(s => (
          <div key={s.label} className="bg-black/20 rounded p-2">
            <div className="flex items-center gap-1 mb-0.5 text-muted">{s.icon}<span className="font-cond text-[9px] font-bold tracking-wider">{s.label}</span></div>
...
// ─── Lightning panel ──────────────────────────────────────────
function LightningPanel({ complex, lightningStatus: ls, onTrigger, onLift }: {
  complex: Complex
  lightningStatus?: LightningStatus
  onTrigger: () => void
  onLift: () => void
  ...
  return (
    <div className="mt-4 pt-4 border-t border-border">
      ...
      <div className={cn(
        ...
        {ls?.active ? '⚡ LIGHTNING DELAY ACTIVE — ALL FIELDS SUSPENDED' : 'FIELD STATUS: ALL CLEAR'}
      </div>
      <button
...
function AlertCard({ alert, onResolve }: { alert: any; onResolve: () => void }) {
  ...
  return (
    <div className={cn(
      ...
      isCritical ? 'border-l-red-500 bg-red-900/10 border-red-900/40' :
      alert.severity === 'warning' ? 'border-l-yellow-500 bg-yellow-900/10 border-yellow-900/30' :
                                     'border-l-blue-400 bg-blue-900/10 border-blue-900/30'
    )}>
      <div className="flex justify-between items-start mb-1">
        <span className={cn('font-cond font-black text-[11px] tracking-widest',
          isCritical ? 'text-red-400' : alert.severity === 'warning' ? 'text-yellow-400' : 'text-blue-300'
        )}>
          {isLightning ? '⚡ ' : isHeat ? '🌡 ' : '⚠ '}{alert.alert_type?.toUpperCase()}
        </span>
...
// ─── Protocol section ─────────────────────────────────────────
function ProtocolSection({ title, color, steps }: { title: string; color: string; steps: string[] }) {
  const [open, setOpen] = useState(true)
  const borderColor = { red: 'border-red-500', yellow: 'border-yellow-500', orange: 'border-orange-400', blue: 'border-blue-400' }[color] ?? 'border-border'
  return (
    <div className={cn('border-l-4 bg-surface-card border border-border rounded-md mb-4 overflow-hidden', borderColor)}>
      <button
          ...
          <ol className="space-y-2">
            {steps.map((step, i) => (
              <li key={i} className="flex gap-3 text-[12px]">
                <span className="font-cond font-black text-muted shrink-0 w-5 text-right">{i + 1}.</span>
